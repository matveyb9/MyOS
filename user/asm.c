...
#define NATIVE_INSTRUCTION_ROL UINT64_C(24)
...
#define NATIVE_INSTRUCTION_DEC UINT64_C(29)
#define NATIVE_INSTRUCTION_SWAP UINT64_C(30)
#define NATIVE_INSTRUCTION_TEST UINT64_C(31)
#define NATIVE_INSTRUCTION_PARITY UINT64_C(32)
#define NATIVE_INSTRUCTION_CLZ UINT64_C(33)
    ...
    while (position < source_length) {
        ...
        if (word_is(source_buffer, start, word_length, "write") != 0) {
            ...
            condition_ready = 1;
                   ...
                   || word_is(source_buffer, start, word_length, "mod") != 0
                   ...
                   || word_is(source_buffer, start, word_length, "rol") != 0
                   || word_is(source_buffer, start, word_length, "ror") != 0
                   || word_is(source_buffer, start, word_length, "test") != 0) {
            struct native_instruction *instruction = &instructions[*instruction_count];
            ...
            if (parse_decimal(source_buffer, source_length, &position, &instruction->condition_value) == 0
                ...
                || ((word_is(source_buffer, start, word_length, "div") != 0
                     || word_is(source_buffer, start, word_length, "mod") != 0)
                    && instruction->condition_value == 0U)
                || ((word_is(source_buffer, start, word_length, "shl") != 0
                     || word_is(source_buffer, start, word_length, "shr") != 0
                     || word_is(source_buffer, start, word_length, "rol") != 0
                     || word_is(source_buffer, start, word_length, "ror") != 0)
            ...
            } else if (word_is(source_buffer, start, word_length, "mod") != 0) {
                instruction->kind = NATIVE_INSTRUCTION_MOD;
            ...
            } else if (word_is(source_buffer, start, word_length, "shr") != 0) {
                instruction->kind = NATIVE_INSTRUCTION_SHR;
            } else if (word_is(source_buffer, start, word_length, "rol") != 0) {
                instruction->kind = NATIVE_INSTRUCTION_ROL;
            } else if (word_is(source_buffer, start, word_length, "ror") != 0) {
                instruction->kind = NATIVE_INSTRUCTION_ROR;
            } else {
                instruction->kind = NATIVE_INSTRUCTION_TEST;
            }
                   ...
                   || word_is(source_buffer, start, word_length, "dec") != 0
                   || word_is(source_buffer, start, word_length, "parity") != 0
                   || word_is(source_buffer, start, word_length, "clz") != 0) {
            if (condition_ready == 0) { return 0; }
            instructions[*instruction_count].kind = word_is(source_buffer, start, word_length, "not") != 0
                                                 ? NATIVE_INSTRUCTION_NOT
                                                 : (word_is(source_buffer, start, word_length, "neg") != 0
                                                    ? NATIVE_INSTRUCTION_NEG
                                                    : (word_is(source_buffer, start, word_length, "inc") != 0
                                                       ? NATIVE_INSTRUCTION_INC
                                                       : (word_is(source_buffer, start, word_length, "dec") != 0
                                                          ? NATIVE_INSTRUCTION_DEC
                                                          : (word_is(source_buffer, start, word_length, "parity") != 0
                                                             ? NATIVE_INSTRUCTION_PARITY : NATIVE_INSTRUCTION_CLZ))));
                   ...
                   || word_is(source_buffer, start, word_length, "swap") != 0) {
            struct native_instruction *instruction = &instructions[*instruction_count];

            if ((word_is(source_buffer, start, word_length, "cmp") != 0
                 || word_is(source_buffer, start, word_length, "swap") != 0) && condition_ready == 0) { return 0; }
            skip_inline_space(source_buffer, source_length, &position);
            if (parse_decimal(source_buffer, source_length, &position, &instruction->condition_value) == 0
            ...
            if (word_is(source_buffer, start, word_length, "store") != 0) {
                ...
                condition_ready = 1;
            } else if (word_is(source_buffer, start, word_length, "swap") != 0) {
                instruction->kind = NATIVE_INSTRUCTION_SWAP;
            } else {
...
static uint64_t instruction_size(uint64_t kind) {
    ...
    if (kind == NATIVE_INSTRUCTION_SWAP) { return UINT64_C(7); }
    if (kind == NATIVE_INSTRUCTION_TEST) { return UINT64_C(9); }
    if (kind == NATIVE_INSTRUCTION_PARITY) { return UINT64_C(8); }
    if (kind == NATIVE_INSTRUCTION_CLZ) { return UINT64_C(21); }
    if (kind == NATIVE_INSTRUCTION_NOT || kind == NATIVE_INSTRUCTION_NEG
        || kind == NATIVE_INSTRUCTION_INC || kind == NATIVE_INSTRUCTION_DEC) { return UINT64_C(2); }
    ...
    for (uint64_t index = 0U; index < instruction_count; index++) {
        ...
        if (instructions[index].kind == NATIVE_INSTRUCTION_WRITE) {
            ...
            elf_buffer[code++] = instructions[index].kind == NATIVE_INSTRUCTION_NOT ? 0xD3U : 0xDBU;
                   ...
                   || instructions[index].kind == NATIVE_INSTRUCTION_DEC) {
            elf_buffer[code++] = 0xFEU;
            elf_buffer[code++] = instructions[index].kind == NATIVE_INSTRUCTION_INC ? 0xC3U : 0xCBU;
        } else if (instructions[index].kind == NATIVE_INSTRUCTION_CLZ) {
            elf_buffer[code++] = 0x84U; elf_buffer[code++] = 0xDBU;
            elf_buffer[code++] = 0x75U; elf_buffer[code++] = 4U;
            elf_buffer[code++] = 0xB3U; elf_buffer[code++] = 8U;
            elf_buffer[code++] = 0xEBU; elf_buffer[code++] = 13U;
            elf_buffer[code++] = 0x0FU; elf_buffer[code++] = 0xBDU; elf_buffer[code++] = 0xCBU;
            elf_buffer[code++] = 0xB8U; put_u32(elf_buffer, code, 7U); code += 4U;
            elf_buffer[code++] = 0x29U; elf_buffer[code++] = 0xC8U;
            elf_buffer[code++] = 0x0FU; elf_buffer[code++] = 0xB6U; elf_buffer[code++] = 0xD8U;
        } else if (instructions[index].kind == NATIVE_INSTRUCTION_PARITY) {
            elf_buffer[code++] = 0x84U; elf_buffer[code++] = 0xDBU;
            elf_buffer[code++] = 0x0FU; elf_buffer[code++] = 0x9AU; elf_buffer[code++] = 0xC3U;
            ...
            elf_buffer[code++] = (uint8_t)instructions[index].condition_value;
        } else if (instructions[index].kind == NATIVE_INSTRUCTION_TEST) {
            elf_buffer[code++] = 0xF6U; elf_buffer[code++] = 0xC3U;
            elf_buffer[code++] = (uint8_t)instructions[index].condition_value;
            ...
            elf_buffer[code++] = 0x0FU; elf_buffer[code++] = 0xB6U; elf_buffer[code++] = 0xDBU;
                   ...
                   || instructions[index].kind == NATIVE_INSTRUCTION_ROL
                   || instructions[index].kind == NATIVE_INSTRUCTION_ROR) {
            elf_buffer[code++] = 0xC0U;
            if (instructions[index].kind == NATIVE_INSTRUCTION_SHL) {
                elf_buffer[code++] = 0xE3U;
            } else if (instructions[index].kind == NATIVE_INSTRUCTION_SHR) {
                elf_buffer[code++] = 0xEBU;
            } else if (instructions[index].kind == NATIVE_INSTRUCTION_ROL) {
            ...
            elf_buffer[code++] = 0x0FU; elf_buffer[code++] = 0xB6U; elf_buffer[code++] = 0xD8U;
                   ...
                   || instructions[index].kind == NATIVE_INSTRUCTION_MOD) {
            elf_buffer[code++] = 0x89U; elf_buffer[code++] = 0xD8U;
            ...
            elf_buffer[code++] = instructions[index].kind == NATIVE_INSTRUCTION_DIV ? 0xD8U : 0xDAU;
        } else if (instructions[index].kind == NATIVE_INSTRUCTION_SWAP) {
            elf_buffer[code++] = 0x86U; elf_buffer[code++] = 0x1CU; elf_buffer[code++] = 0x25U;
...
void _start(uint64_t argc, const char *arguments) {
    ...
    if (argc != 1U || copy_path(source_path, sizeof(source_path), arguments, &argument_position) == 0) {
        ...
        write_text("Source: set <0..255>; not; neg; inc; dec; parity; clz; add/sub/mul/and/or/xor/test <0..255>; shl/shr/rol/ror <1..7>; div/mod <1..255>; store/load/cmp/swap <0..7>; input; time; args; label name:; write \"text\"; jump[_if_zero|_if_nonzero] name; jump_if <0..255> name; exit <0..255>\n");
