...
static const char *const shell_commands[] = {
    "help", "echo", "uname", "sysinfo", "ps", "meminfo", "date", "uptime", "ls", "cat", "cp", "wc", "grep", "find", "head", "sort", "tail", "stat", "touch", "mkdir", "write", "rm",
...
static void command_help(const char *topic) {
    ...
    if (text_equal(topic, "tree")) {
        write_text("run tree [absolute-directory]\n");
    ...
    write_text("Tools: calc <a> <op> <b>; edit <absolute-file>; run tree [absolute-directory]; find <name-fragment> [absolute-directory]; head <absolute-file> [1..64 lines]; stat <absolute-path>; tail <absolute-file> [1..64 lines]; sort <absolute-file>; run <program-or-absolute-path> [arguments]; help cp/tree/find/head/stat/tail/sort/startgui\n");
