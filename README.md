...
Ядро запрашивает у загрузчика framebuffer, карту физической памяти, сведения о прошивке и загрузчике. Затем оно инициализирует собственные GDT и IDT, обработчики 32 процессорных исключений, bitmap-менеджер физических страниц и ограниченный early-paging для MMIO Local APIC. Legacy PIC ремаппируется, Local APIC работает в virtual-wire режиме, а PIT генерирует IRQ0 с частотой 100 Гц. PS/2-клавиатура включает scanning и передаёт US QWERTY символы через IRQ1 в кольцевой буфер. Shell принимает ввод и с COM1, и с PS/2, ожидая события инструкцией `hlt`. Доступны команды `ticks`, `irqs`, `flags` и `keyboard` в дополнение к прежним встроенным командам. Это всё ещё ring 0-прототип: нет USB HID, IOAPIC, пользовательского режима, полноценного виртуального менеджера памяти или framebuffer-терминала.

| Компонент | Решение в версии 0.4.0-dev | Причина |
|---|---|---|
...
Команда `make run` стартует QEMU в BIOS-режиме без графического окна и выводит COM1 в текущий терминал. Ожидаемый результат содержит строку `MyOS 0.4.0-dev — x86_64 kernel`, несколько сообщений `[ok]` и приглашение `myos>`. Наберите `help`, чтобы увидеть доступные встроенные команды.
...
| `0.5.0-dev` | Полный 4-level paging, kernel heap, архитектурный `boot_info` и резервирование страниц ядра. | Ядро безопасно управляет виртуальной памятью и динамическими объектами. |
| `0.6.0-dev` | Текстовая консоль поверх framebuffer, шрифт, строковый редактор и расширенная PS/2-клавиатура. | Пользователь видит и вводит команды на обычном экране без serial-консоли. |
