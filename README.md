...
### Отложенный учебный этап — только после завершения разработки

После того как будет согласована и завершена функциональная версия MyOS, проект перейдёт в отдельный этап **Learning Edition**. До этого момента мы не будем массово переписывать комментарии или документацию: преждевременные пояснения быстро устаревают и мешают разработке.

| Артефакт после завершения разработки | Обязательство |
|---|---|
| Комментарии в исходниках | Объяснить назначение модулей, инварианты, риски, аппаратные предпосылки и причины архитектурных решений; не комментировать очевидный синтаксис. |
| Пошаговое руководство | Описать путь от Limine-загрузки и GDT/IDT до paging, IRQ, драйверов, heap и framebuffer-консоли с воспроизводимыми командами QEMU. |
| Карта архитектуры | Связать каталоги, порядок bootstrap, ownership памяти, обработку ошибок и будущие границы user space. |
| Учебные задания | Добавить изолированные упражнения, контрольные точки и тесты без требования разбираться во всём коде сразу. |
| Версионный снимок | Зафиксировать tagged release и создать отдельную ветку/пакет с учебными материалами. |

> Этот этап **запланирован**, но намеренно не начинается сейчас: текущая задача — сохранять скорость и согласованность разработки ядра.
