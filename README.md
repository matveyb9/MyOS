...
The current stable line is [`console-stable`](docs/RELEASES.md) at [`v0.12.1-console`](docs/RELEASES.md). The `main` branch is the QEMU-validated integration line for graphical and user-program development; it remains **experimental** because physical-PC validation is currently unavailable. New functional work starts in a short-lived feature branch from `main` and returns through a reviewed QEMU validation gate. Any public artifact published from `main` remains a **Pre-release** until a physical-PC validation policy is adopted. This branch includes BIOS/UEFI boot paths, MYPFS004 persistent storage, framebuffer GUI with a bounded mouse-first `startgui` desktop launcher that discovers up to four installed `/apps/<name>/main.elf` tiles, a compositor-owned `HH:MM:SS` clock widget refreshed once per PIT second through a clock-only partial repaint and live bounded `FOCUS`/`TASKS`/`RUN` status footer refreshed with GUI content, per-window title-bar raise and close controls, a general bounded text editor, persistent ELF packages, the MyOS SDK with a bounded public VFS subset, direct shell `cp` backed by the live developer tool, direct bounded `wc` word counting, direct bounded `grep` text search, direct bounded `tree` hierarchy view, direct bounded `find` name search, direct bounded `head` text preview, direct bounded `sort` text ordering, direct bounded `tail` text preview, direct bounded `stat` metadata lookup and packaged `sdk-write` safe-write example, native read-only bounded `tree`, `find`, `head`, `tail`, `sort`, `stat` and 12 KiB `stackprobe` VFS/platform diagnostics, a four-page 16 KiB guarded ring-3 user stack, an in-OS assembler with bounded program-argument forwarding, single-byte input, RTC `HH:MM:SS` output, eight private `store`/`load` byte variables, bounded bitwise `not`, modular byte `neg`, `inc` and `dec`, bounded leading-zero `clz`, normalized even-parity `parity`, `and`, `or` and `xor`, bounded logical `shl`/`shr` byte shifts and circular `rol`/`ror` byte rotates, modular `add`/`sub`/`mul` byte arithmetic, safe unsigned `div`, bounded unsigned-remainder `mod`, and bounded private-slot `cmp` and `swap`, normalized byte-mask `test`, labels, explicit condition values, exact-byte comparison and forward-only unconditional or conditional jumps, a read-only `/system/live/` System Inventory with boot, compiled-in driver, device and process records exposed by `sysinfo`, and File Workspace v1: a compact `FILES` desktop tile that starts at `/users/myos/`, browses the complete logical VFS with its full current path in the window title, fixed-column type and byte-size metadata, creates a named file or directory from bounded slash-free prompts only in existing writable roots, displays and freezes a named file or empty-directory target until a second Enter confirms its removal, copies one regular file up to 64 KiB to an absent same-directory target through bounded VFS chunks, renames an existing mutable file or directory within that same directory without copying its content or moving children, moves one regular file by metadata update to an existing writable destination directory while retaining its basename and rejecting overwrites outside cross-anchor boundaries, opens a new file in the GUI editor and refreshes the browser after a mutation, and opens bounded writable text files up to 16 KiB in the GUI editor through up to sixty-four unchanged 256-byte VFS transfers without weakening read-only or boot boundaries.

| Line | Purpose | State |
|---|---|---|
| `console-stable` | Reviewed console baseline. | Current stable line, tagged `v0.12.1-console`. |
| `main` | QEMU-validated integration line for GUI and user-program development. | Experimental; source for new feature branches and Pre-releases only. |
| `feature/gui` | Historical GUI integration line. | Merged locally into the main-based integration branch; retained until a separate archival decision. |

<h2 align="center">Quick start</h2>

Clone the repository and choose the branch you want to run. The commands below select the current QEMU-validated integration line.
