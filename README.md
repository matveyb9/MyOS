...
Ядро запрашивает у загрузчика framebuffer, карту физической памяти, сведения о прошивке и загрузчике. Затем оно инициализирует собственные GDT и IDT, обработчики 32 процессорных исключений и bitmap-менеджер физических страниц. MyOS выделяет собственный PML4, копирует рабочие записи загрузчика и передаёт управление новому CR3; новые 4 KiB отображения создаются через собственный обход PML4/PDPT/PD/PT. Local APIC использует отдельное некэшируемое MMIO-отображение, а kernel heap лениво отображает физические страницы в выделенном supervisor-only диапазоне. PIC, Local APIC virtual-wire, PIT и PS/2-клавиатура сохраняют работу. Shell поддерживает команды `paging`, `heap` и `heaptest` для диагностики нового слоя памяти. Это всё ещё ring 0-прототип: нет user space, page-fault policy, освобождения heap, USB HID, IOAPIC или framebuffer-терминала.

| Компонент | Решение в версии 0.5.0-dev | Причина |
|---|---|---|
...
| Физическая память | Bitmap до 4 GiB; освобождаются только области `USABLE` из карты Limine. | Выделяет кадры для PML4, таблиц и heap-страниц. |
...
| Kernel heap | 1 GiB supervisor-only диапазон с 16-байтным выравниванием и ленивым отображением страниц. | Будущие драйверы и консоль получают безопасную динамическую память. |
...
Команда `make run` стартует QEMU в BIOS-режиме без графического окна и выводит COM1 в текущий терминал. Ожидаемый результат содержит строку `MyOS 0.5.0-dev — x86_64 kernel`, несколько сообщений `[ok]` и приглашение `myos>`. Наберите `help`, чтобы увидеть доступные встроенные команды.
...
| `0.6.0-dev` | Резервирование страниц ядра, policy page fault, свободный список heap и изоляция будущего user space. | Отображения могут безопасно изменяться и освобождаться без утечек раннего ядра. |
| `0.7.0-dev` | Текстовая консоль поверх framebuffer, шрифт, строковый редактор и расширенная PS/2-клавиатура. | Пользователь видит и вводит команды на обычном экране без serial-консоли. |
| `0.8.0-dev` | Планировщик, ring 3, системные вызовы, ELF и initramfs. | Минимальная пользовательская программа выполняется из initramfs. |
| `0.9.0-dev` | VFS и простая постоянная файловая система. | Система монтирует файлы и запускает программу по имени. |
