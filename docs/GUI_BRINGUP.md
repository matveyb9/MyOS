...
| Save and cancel | `Ctrl-S` replaces the selected file in `/users/myos/files/notes/`, writes the draft and returns to its viewer. `Esc`, `Alt+F4` on focused NOTES, or the `NOTES` window `X` cancels the draft and reloads the previously saved content. |
