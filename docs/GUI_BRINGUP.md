...
| Windows | Outside launcher mode, three static bounded window records: `SYSTEM`, `NOTES` and `MONITOR`. |
...
| Focus | Clicking an ordinary visible window raises it; `Tab`, `Enter` or `Space` outside the editor move focus to the next visible window. |
| Hardware pointer | PS/2 mouse relative motion moves a bounded crosshair pointer. A rising-edge left click activates a launcher tile or top-bar `X`; otherwise it raises the topmost visible window under the pointer. |
...
| Mouse hardware | PS/2 relative motion and a left-button edge are implemented. In launcher mode, the kernel maps only three fixed tile rectangles and the fixed top-bar `X` rectangle to bounded action characters delivered through the existing scheduler-safe input queue; ordinary viewer clicks focus windows. Motion remains cursor-only; dragging, wheel and multi-button semantics are not present. |
