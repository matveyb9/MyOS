...
| Launcher | In desktop-home mode, three fixed clickable tiles — `SYSTEM`, `NOTES` and `EDIT NOTE` — plus up to four discovered `/apps/<name>/main.elf` package tiles replace the ordinary windows. |
...
| Persistent selection | Up to 64 scanned directory indices | The GUI uses `MYOS_SYS_VFS_LIST` only in the notes directory. |
