...
| Built-in choices | Clicking `SYSTEM`, `NOTES` or `EDIT NOTE` dispatches the same bounded actions as `M`, `N` and `E`; `H` returns to MYOS DESKTOP, and desktop-home `N` opens the bounded personal-notes route. |
| Focus | Clicking an exposed ordinary window title bar raises it; a body click also retains the existing topmost-window focus behavior. `Tab`, `Enter` or `Space` outside the editor move focus to the next visible window. |
...
| Keyboard fallback | `M`/`N`/`E`/`H`/`Q` mirror launcher actions; lowercase `W`, `A`, `S`, `D` move the pointer by 16 pixels and `F` locks keyboard focus to the top window under the pointer. |
| Visibility | `1`, `2`, `3` toggle `SYSTEM`, `NOTES`, `MONITOR`; `X` hides the focused window, with no option to hide all windows. |
| Reset and exit | `R` restores the original layout and z-order. `Q` or `Esc` outside the editor ends the session and returns to the framebuffer text console. |

The letter `D` is used only in uppercase to select `/users/myos/files/notes/note`; `N` or `n` cycles the next existing note inside `/users/myos/files/notes/`. This preserves lowercase `d` as pointer-right movement. In the editor ordinary printable keys become draft text and are not forwarded to the window manager, so `D`, `N`, `Q` and other characters can be typed as part of a note.
...
| Input | Existing scheduler-safe console input path; the PS/2 auxiliary port emits three-byte packets via IRQ12. PS/2 arrows, Home, End and Delete are translated into internal bounded key bytes; in the editor keys belong to the draft, not the window manager. |
...
| Automated `make regression` | Passed: the disposable-image harness created/edited/saved a BIOS GUI note, navigated default `startgui` through M/H/N/H/Q and returned cleanly, then used QMP to inject PS/2 mouse events for the centered `NOTES` tile, `SYSTEM` and `MONITOR` window `X` controls, MONITOR title-bar raise, viewer close-to-home and editor cancel-to-viewer. PPM framebuffer transitions validate each visual change in BIOS and UEFI. The harness also verifies the retained `startgui home` alias, copies a paced editor-authored 305-byte file with SDK `cp` across the VFS request boundary, verifies exact data and overwrite refusal, builds/installs/runs legacy native packages, checks empty and forwarded `args` output, exact `input` match and fallback paths plus valid RTC `HH:MM:SS` output, rejects invalid forward-only control flow, then UEFI repeats desktop-home navigation, reads persisted files and copied data, reruns installed input/time/argument packages and enters/exits GUI cleanly. See [RELEASE_STABILIZATION.md](RELEASE_STABILIZATION.md). |
