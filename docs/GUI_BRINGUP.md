...
| `struct myos_gui_content_request` | 4,240 bytes | `length`, `flags`, `cursor`, `viewport`, `title[112]`, `data[4096]`; copied only by the active GUI-session path after a dedicated mapped-range validation. The ordinary 512-byte syscall I/O limit is unchanged. |
