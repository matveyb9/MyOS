...
> **Language:** [English](GUI_BRINGUP.md) | [Русский](GUI_BRINGUP_RU.md)
...
`startgui` is an ordinary ring-3 program. Without arguments it opens the bounded **MYOS DESKTOP** home view; `startgui home` remains a compatibility alias. Its mouse-first launcher exposes clickable `SYSTEM`, `NOTES` and `EDIT NOTE` tiles, plus a top-bar `X` exit control. In viewer or editor mode the active `NOTES` surface is raised above the static windows; click any exposed window title bar to raise it. Each window also has its own `X`: `SYSTEM` and `MONITOR` are hidden, the `NOTES` viewer returns home, and the `NOTES` editor cancels its unsaved draft and restores the selected-file viewer. The separate top-bar `X` still ends the GUI session. GUI-level keyboard fallback is deliberately compact: `Alt+Tab` moves focus to the next visible window, `Ctrl+B` returns to MYOS DESKTOP, and `Alt+F4` exits to the same user shell. The launcher and window actions are otherwise mouse-only. The editor retains `Ctrl+S` to save and `Esc` to cancel; `Ctrl+B` or `Alt+F4` from the editor explicitly discards its unsaved draft before returning home or exiting. It creates a restricted GUI session through the existing syscall boundary, reads and edits only bounded user-space payloads, and the kernel receives only validated syscall requests. `startgui /users/myos/files/notes/<name>` opens a read-only viewer for that selected persistent note; use the `EDIT NOTE` tile for the default personal note or the general `edit <absolute-file>` command for ordinary text files.

## Persistent user programs

The GUI branch can now launch separate MyOS ELF64 files from global persistent application packages at `/apps/<name>/main.elf`. First the built-in initramfs program can be copied into a package, after which it can be launched by a short shell name or an absolute path as a separate ring-3 process.
...
| Desktop | Dark desktop, a top status bar with a clickable `X` exit control, and a bottom row of available controls. |
| Launcher | In desktop-home mode, three fixed clickable tiles — `SYSTEM`, `NOTES` and `EDIT NOTE` — replace the ordinary windows. |
...
| Desktop home | Bare `startgui` renders the fixed mouse-first `MYOS DESKTOP` launcher; `startgui home` is a compatibility alias. Only the three fixed tile rectangles and top-bar exit rectangle are actionable; it does not scan arbitrary paths, create tasks or retain unbounded state. |
...
| Editor entry | The `EDIT NOTE` tile opens a bounded editor for the default personal note; a missing default path starts as an empty draft. |
...
| Save and cancel | `Ctrl-S` replaces the selected file in `/users/myos/files/notes/`, writes the draft and returns to its viewer. `Esc` or the `NOTES` window `X` cancels the draft and reloads the previously saved content. |
| Built-in choices | Clicking `SYSTEM`, `NOTES` or `EDIT NOTE` dispatches fixed bounded actions. `NOTES` opens the bounded personal-notes route and `EDIT NOTE` opens the default personal-note editor. |
...
| Keyboard fallback | `Alt+Tab` cycles focus, `Ctrl+B` returns home and `Alt+F4` exits. Temporary single-letter GUI commands, keyboard pointer movement, numeric visibility toggles and layout reset are removed. |
...
| Reset and exit | `Ctrl+B` returns home. `Alt+F4` or the top-bar `X` ends the session and returns to the framebuffer text console; `Esc` remains editor-cancel only. |

In the editor ordinary printable keys become draft text and are not forwarded to the window manager. `Ctrl+S` saves, `Esc` cancels to the viewer, and the explicitly global `Ctrl+B`/`Alt+F4` paths discard the draft before returning home or exiting.

## Editor limits and ABI boundary

`NOTES` uses the descriptor `MYOS_GUI_SET_CONTENT = 3` in `MYOS_SYS_GUI_SESSION`. The request accepts either editable content or the fixed launcher-content flag, never both; only the latter enables the renderer's three static launcher hit rectangles. The kernel accepts content requests only from the current GUI owner with an active session, copies the request after validating the user buffer mapping and does not retain user pointers. The framebuffer owns its own static copies of title and data. The GUI periodically uses directory-scoped `MYOS_SYS_VFS_LIST` for `/users/myos/files/notes/` and keeps the selected absolute path in bounded static storage. The editor removes the selected file, creates it via the unified VFS and writes one bounded payload at offset `0`.
...
| Persistent selection | Up to 64 scanned directory indices | `N` uses `MYOS_SYS_VFS_LIST` only in the notes directory. |
| Selected path | Up to 111 ASCII bytes plus NUL | The GUI stores absolute note paths in `/users/myos/files/notes/`; `/apps/` is intended for executable workflow. |
| Persistent save | Up to 128 bytes per editor update | `VFS_REMOVE`, `VFS_CREATE_FILE`, then bounded `VFS_WRITE`; the GUI editor intentionally does not edit binary ELF files. |
...
| Automated `make regression` | Passed: the disposable-image harness creates and saves the default GUI note through the mouse `EDIT NOTE` tile, verifies QMP PS/2 `Alt+Tab` focus at MONITOR, `Ctrl+B` desktop return and `Alt+F4` exit, then uses mouse events for the centered `NOTES` tile, `SYSTEM` and `MONITOR` window `X` controls, MONITOR title-bar raise, viewer close-to-home and editor cancel-to-viewer. PPM framebuffer transitions validate the visual steps in BIOS and UEFI. The harness also verifies the retained `startgui home` alias, copies a paced editor-authored 305-byte file with SDK `cp` across the VFS request boundary, verifies exact data and overwrite refusal, builds/installs/runs legacy native packages, checks empty and forwarded `args` output, exact `input` match and fallback paths plus valid RTC `HH:MM:SS` output, rejects invalid forward-only control flow, then UEFI reads persisted files and copied data, reruns installed input/time/argument packages and enters/exits GUI cleanly. See [RELEASE_STABILIZATION.md](RELEASE_STABILIZATION.md). |
