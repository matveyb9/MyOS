...
# Project Hub: startgui projects
# Project Workbench: startgui project <name>
...
| Project Hub and global overview | `startgui projects` pages only valid project directories from the fixed project root and opens a selected revalidated entry in Project Workbench; it uses the existing browser action surface for refresh, paging, global status and raw project-root return. `startgui projects status` remains read-only: it scans at most 128 root entries, filters only valid project directories, and renders only fixed source/build/package rows without mutation or child processes. |
| Direct project creation | `startgui project <name> new [hello|args|empty]` accepts only the bounded name and fixed starter selection, including the editable zero-length `empty` source, creates only the fixed workspace/source pair, rejects an existing target, and rolls back only its own partial creation state. GUI stays active with a narrow result status and no child process. |
...
Bare `startgui project <name>` now opens **Project Workbench** rather than a generic project file listing. It keeps the existing 1–31-character ASCII project-name validation and displays the fixed `source`, `build`, and `package` state for only `/users/myos/projects/<name>/main.mya`, `/users/myos/projects/<name>/main.elf`, and `/apps/<name>/main.elf`.

The bounded clickable actions are **Refresh**, **Edit source**, **Build**, **Run**, **Install**, **Uninstall**, **Clean build**, **Remove workspace**, and **Status**. They reuse the already established editor, assembler, loader, installer, and one-path removal helpers. Build, run, and install hand off to their existing console child; run from Workbench accepts no free-form tail, while the direct `startgui project <name> run [arguments]` form retains the bounded 127-visible-byte native argument tail. The Workbench adds no VFS primitive, arbitrary path, executable path, or loader allowance. A failed action redraws the same selected workspace with a narrow status; a successful workspace removal returns to the project-root browser, still requires the exact clean-workspace condition, and preserves the installed package.
