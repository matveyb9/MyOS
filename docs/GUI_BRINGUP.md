...
| File Workspace | `FILES` begins at `/users/myos/`, supports parent, directory-entry and paged navigation across the complete logical VFS without ending the GUI session, and identifies directories, regular files and virtual entries. Every row has a fixed 12-character visible-name column, a `D`/`F`/`L`/`V` type marker and current logical-VFS byte size with a `B` suffix. The `[NEW FILE]`, `[NEW FOLDER]`, `[DELETE]` and `[COPY]` rows are active only in `/users/myos`, `/temp`, `/system/data` and `/system/config`: each accepts one non-empty printable ASCII name of up to 63 bytes excluding `/`. The first creates a new empty file through the unified VFS and opens the bounded GUI editor; the second creates a new empty directory and refreshes the browser in place. `[DELETE]` displays and freezes the full named target, then invokes the existing remove operation only after a second `Enter`; `Esc` cancels before mutation. `[COPY]` accepts source and target names in two prompts, copies only a regular source to an absent target in the same current directory, streams no more than 64 KiB in existing 256-byte VFS requests and removes only its own partial target on a copy failure. Read-only roots, invalid names, existing creation paths, non-empty deletion directories and non-regular or oversized copy sources report a status without mutation. |
...
| Editor entry | The `EDIT NOTE` tile opens a bounded editor for the default personal note; a missing default path starts as an empty draft. FILES opens an existing regular file in the same editor only when its VFS path is writable, and its `[NEW FILE]` control creates then opens a new empty regular file, while `[NEW FOLDER]` creates and lists a new empty directory under the same writable-root policy, and `[DELETE]` removes a named file or empty directory only after it shows that frozen target and receives a second `Enter`; `[COPY]` streams an existing regular file of at most 64 KiB to a new same-directory target without overwriting it. |
