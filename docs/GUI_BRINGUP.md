...
| File Workspace | `FILES` begins at `/users/myos/`, supports parent, directory-entry and paged navigation across the complete logical VFS without ending the GUI session, and identifies directories, regular files and virtual entries. Every row has a fixed 12-character visible-name column, a `D`/`F`/`L`/`V` type marker and current logical-VFS byte size with a `B` suffix. The `[NEW FILE]` and `[NEW FOLDER]` rows are active only in `/users/myos`, `/temp`, `/system/data` and `/system/config`: each accepts one non-empty printable ASCII name of up to 63 bytes excluding `/`. The first creates a new empty file through the unified VFS and opens the bounded GUI editor; the second creates a new empty directory and refreshes the browser in place. Read-only roots, invalid names and existing paths report a status without mutation. |
