...
| Heap free/reuse | Passed | Passed | After `kfree` the address of the first 64-byte block is reallocated by the allocator. |
| Heap diagnostics | Passed | Passed | After the test `active allocations = 0`, `free blocks = 1`, `reuses = 1`. |
| Page fault CR2 | Passed | Passed | Controlled unmapped access reports `0xFFFF900040000000`. |
...
The `pagefault` command reads an address just beyond the allocated heap virtual range. It should stop the kernel and produce the following human-readable report:
...
The processor saves the faulting linear address in CR2, and the bits of the error code describe presence, write/read, user/supervisor, reserved-bit and instruction-fetch properties of the fault. MyOS reads CR2 before printing diagnostics because the next page fault could overwrite this register. [1]

| Component | Guarantee 0.6.0-dev | Limitation |
|---|---|---|
| PMM | `free` rejects an unaligned, unsuitable, already-free, or out-of-range frame. | No owner tags and no memory above 4 GiB. |
| Heap | `kfree` rejects NULL, non-heap addresses, double free, bad magic, or bad size. | No lock, red-zone, or page reclamation. |
| Paging | New heap/MMIO pages are mapped through the custom PML4. | No `unmap`, NX, or user mappings. |
...
In a normal shell run `pmmtest`, `heaptest`, `heap`, `paging`, `ticks` and `irqs`. The `pagefault` command must be run separately because the correct behavior ends with a diagnostic halt of the kernel.
