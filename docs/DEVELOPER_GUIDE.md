...
| `make regression` | Disposable-image QMP PS/2 `Alt+Tab` focus, `Alt+F4` close of focused MONITOR, `Esc` viewer return, `Alt+F4` editor cancel-to-viewer and `Ctrl+Q` clean exit pass in BIOS and UEFI; mouse activation of the centered NOTES tile, SYSTEM/MONITOR close controls, MONITOR title-bar raise, viewer close-to-home and editor cancel-to-viewer also pass with PPM framebuffer transitions. The retained `startgui home` alias, GUI note editing, a paced editor-authored 305-byte SDK `cp` copy across the VFS chunk boundary, exact readback and overwrite rejection, editor source workflow, legacy zero/nonzero branches, empty and forwarded native `args`, native `input` exact-match/fallback paths, valid RTC `HH:MM:SS` output and rejected targets also pass. UEFI repeats the full GUI modifier/mouse surface, persisted text/copied-target readback, package execution and clean GUI return. |
