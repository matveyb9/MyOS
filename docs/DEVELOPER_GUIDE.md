...
| `make regression` | Disposable-image BIOS `startgui home` navigation through M/H/N/H/Q and clean return, GUI note editing, a paced editor-authored 305-byte SDK `cp` copy across the VFS chunk boundary, exact readback and overwrite rejection, editor source workflow, legacy zero/nonzero branches, empty and forwarded native `args`, native `input` exact-match/fallback paths, valid RTC `HH:MM:SS` output and rejected targets pass; UEFI repeats desktop-home navigation, reads persisted text and copied target, runs persisted packages including input/time/argument programs, and returns cleanly from GUI. |
