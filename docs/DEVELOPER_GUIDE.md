...
`newproj <name>` is the bounded project entry point. It accepts exactly one 1–31-character ASCII letter/digit/`-`/`_` component, creates `/users/myos/projects/<name>/main.mya` from one fixed runnable template, never overwrites an existing project and removes only its own partial setup state on a later ordinary setup failure. `editproj <name>` constructs the fixed source/editor path (`main.mya`), `buildproj <name>` constructs that source/output pair (`main.mya` → `main.elf`), while `installproj <name>` constructs that output/package pair (`main.elf` → `/apps/<name>/main.elf`), `projstatus <name>` reports the three fixed source/build/package entries through at most 128 VFS-list probes per parent, and `cleanproj <name>` removes only an existing regular fixed build output (`main.elf`) after revalidation; the first three delegate to the established `edit`, `asm` and `install` programs, preserve their error behavior and retain the installer’s explicit package-replacement behavior. The commands use existing VFS requests, keep source and installed package untouched during cleanproj, and do not claim crash-transactional persistence. BIOS regression checks template bytes, duplicate rejection, source-ready/build-package-missing status, shortcut editor entry, shortcut build/install, ready build/package status, clean/rebuild behavior, package preservation and execution; UEFI checks persisted template, package, package-ready status and the final missing build output.
