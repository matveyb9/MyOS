...
| `main` | Experimental QEMU-validated integration branch for GUI and user-program work. Create every new functional change in a short-lived branch from this ref; merge only after reviewed QEMU validation. |
...
A normal source publication may push `main` and `console-stable`. Until physical-PC validation policy changes, every public artifact from `main` is a separately approved **Pre-release**, never a stable Release. Build artifacts `myos.iso` and `myos.img` are release attachments, not Git-tracked source files.
