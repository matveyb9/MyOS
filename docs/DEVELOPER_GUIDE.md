...
The release includes write/read, process lifecycle, task info, VFS read/enumeration/create/write/remove/rename/move, RTC/uptime, bounded spawn arguments, tmpfs/persistent-file operations, pipe operations, reboot and poweroff. The executable loader allowlist accepts `/system/core/apps/...`, installed `/apps/<name>/main.elf` packages, and only the exact bounded development output `/users/myos/projects/<name>/main.elf`, where `<name>` is 1–31 ASCII letters, digits, `-` or `_`; it rejects every other user-controlled path. The most important limits are:
...
The bounded mouse-first desktop launcher entered by bare `startgui` (with `startgui home` as an alias, `startgui projects` as bounded Project Hub entry for `/users/myos/projects` and bare `startgui project <name>` as a direct revalidated existing `/users/myos/projects/<name>` **Project Workbench** entry (only 1–31 ASCII letter/digit/`-`/`_` names; its exact `edit` suffix revalidates regular writable `main.mya` before entering the existing editor; its exact read-only `status` suffix reports only fixed source/build/package regular-file state through at most 128 probes per parent; its exact `build` suffix revalidates only fixed regular `main.mya`, passes only that source and fixed `main.elf` output to the established assembler, ends GUI for console completion, and returns the assembler status through `startgui`; its exact `run [arguments]` suffix revalidates only fixed regular `main.elf`, launches only that already allowlisted output with at most the existing 127 visible-byte native tail, then returns the child status after ending GUI; its exact `install` suffix revalidates only that fixed regular output, invokes the established installer only for fixed `/apps/<name>/main.elf`, returns installer status after ending GUI, and preserves package replacement; its exact `uninstall` suffix revalidates only fixed regular `/apps/<name>/main.elf`, removes only that package output while keeping source/build and GUI active, and spawns no child; its exact `clean` suffix revalidates only fixed regular `<project>/main.elf`, removes only that build output while keeping source/package and GUI active, and spawns no child; its exact `new [hello|args|empty]` suffix accepts only a bounded absent name and fixed starter selection, including editable zero-length `empty`, creates only fixed `main.mya`, rejects an existing directory, and rolls back only its own partial source/directory state after later ordinary failure; its exact `new [hello|args|empty] edit` suffix uses the same bounded create-only grammar, then selects only the newly created fixed source for the existing editor after success; an ordinary editor-handoff failure preserves that completed workspace; its exact `new [hello|args|empty] build` suffix uses the same bounded create-only grammar, then after success invokes the established assembler only for that new fixed `main.mya → main.elf` pair, ends GUI for console completion and preserves the completed workspace after an assembler failure; its exact `new [hello|args|empty] install` suffix uses the same bounded create-only grammar, then invokes the established assembler only for that new fixed source/output pair and only after a regular build result invokes the established installer only for its matching package, ending GUI for child completion while preserving the completed workspace after a build or installer failure; its exact `remove` suffix immediately revalidates the exact project directory, allows only regular `main.mya` when present and absent `main.elf`, removes source then empty directory, preserves the installed package, stays in GUI and does not claim crash-transactional deletion; invalid/absent project, source, output or package targets report without viewer fallback)) has four compact fixed tiles including File Workspace v1; `FILES` starts at `/users/myos/`, shows its full current logical path in the window title, traverses the logical VFS in four-entry pages, exposes read-only virtual/system records safely and offers bounded `[NEW FILE]`, `[NEW FOLDER]`, `[DELETE]`, `[COPY]`, `[RENAME]` and file-only `[MOVE]` actions in writable roots, plus bounded read-only `[SEARCH]` in every browsable root, opens the new file in the completed 16 KiB GUI editor, refreshes after a new directory, requires a named second-Enter confirmation before removing a file or empty directory, streams one regular file of at most 64 KiB to an absent same-directory target without overwriting it, and renames one existing mutable object only within its current directory without copying content or moving its children, and moves one regular file by metadata update to an existing writable absolute directory without overwrite, retaining its basename and staying within one persistent move anchor or `/temp`. Up to four verified `/apps/<name>/main.elf` package tiles, each requiring a printable name of at most 15 characters in both the framebuffer and user-space resolver, per-window title-bar raise and close controls, a bounded `FOCUS HOME/SYSTEM/NOTES/MONITOR` footer indicator and standard `Alt+Tab`/`Alt+F4`/`Esc`/`Ctrl+Q` keyboard fallback without legacy single-letter GUI commands, SDK VFS subset, direct shell `cp`, `wc`, `grep`, `tree`, `find`, `head`, `sort`, `tail` and `stat` backed by the live developer tools, native build, bounded control-flow, argument forwarding, input/time and general text-editor milestones are complete: `asm` emits an x86_64 `ET_EXEC` with a fixed private data segment from `.mya` source; shell `build` provides the project workflow; `args`, `input`, `time`, `set <0..255>`, `not`, `neg`, `inc`, `dec`, `parity`, `test <0..255>`, `and <0..255>`, `or <0..255>`, `xor <0..255>`, `shl <1..7>`, `shr <1..7>`, `rol <1..7>`, `ror <1..7>`, `add <0..255>`, `sub <0..255>`, `mul <0..255>`, `div <1..255>`, `mod <1..255>`, `store <0..7>`, `load <0..7>`, `cmp <0..7>`, `swap <0..7>`, `label name:`, `jump name`, `jump_if_zero name`, `jump_if_nonzero name` and `jump_if <0..255> name` compile to bounded forward-only code with eight private byte variables. `not`/`and`/`or`/`xor` require the initialized byte accumulator and update its eight bits; operand-free `neg` has the same prerequisite and replaces it with the two’s-complement result modulo 256; operand-free `inc` has the same prerequisite and increments it modulo 256; operand-free `dec` has the same prerequisite and decrements it modulo 256; operand-free `parity` normalizes its even byte parity to one or odd parity to zero; `test <0..255>` has the same prerequisite and normalizes its byte-wise intersection to zero or one; `shl`/`shr` require it and logically shift the byte by 1..7 positions; `rol`/`ror` require it and circularly rotate the byte by the same range; `add`/`sub`/`mul` also require it and wrap modulo 256; `div` has the same prerequisite, rejects zero and retains an unsigned integer quotient; `mod` has the same prerequisite, rejects zero and retains the unsigned remainder; `cmp` compares that accumulator with a private slot and turns equality into zero or inequality into one; `swap` requires the same initialized accumulator and exchanges its byte with one private slot. Direct `edit <absolute-file>` provides cursor-based multi-line editing for ordinary files and `.mya` source, with a 4 KiB all-in-memory document limit; its contract is in [TEXT_EDITOR.md](TEXT_EDITOR.md). Future native work must preserve the established storage, ABI and control-flow limits; no C frontend or general linker is planned before those limits are stabilized. Do not merge GUI, MYPFS004 or native-toolchain work into `console-stable` without an explicit maintenance decision; new work returns to `main` only through the documented QEMU validation workflow.

`newproj <name> [hello|args|empty] [edit|build|run|install]` is the bounded project entry point. It accepts one 1–31-character ASCII letter/digit/`-`/`_` component plus an optional exact starter name: `hello` is the default runnable template, `args` is the fixed bracketed native-argument template, and `empty` creates an editable regular zero-length source. Only exact final `edit`, `build`, `run` or `install` is accepted after that selection. It validates both before VFS mutation, creates `/users/myos/projects/<name>/main.mya`, never overwrites an existing project and removes only its own partial setup state on a later ordinary setup failure. After success, `edit` runs only against that new fixed source, `build` invokes the established fixed `main.mya → main.elf` project pair, `run` first revalidates that new build output is regular before invoking only that established bounded project output, and `install` first revalidates that new build output is regular before invoking the established fixed `main.elf → /apps/<name>/main.elf` pair; a later editor-launch, build, run or install failure preserves the completed project. `editproj <name>` constructs the fixed source/editor path (`main.mya`), `buildproj <name>` constructs that source/output pair (`main.mya` → `main.elf`), `runproj <name> [arguments]` constructs the fixed generated output path, revalidates it is regular and delegates to the existing foreground loader without mutation, forwarding at most the existing 127 visible-byte native argument tail, while `installproj <name>` constructs that output/package pair (`main.elf` → `/apps/<name>/main.elf`), `uninstallproj <name>` revalidates and removes only an existing regular fixed installed output (`/apps/<name>/main.elf`) while preserving source/build and reporting absence without mutation, `projlist` is read-only, enumerates at most 128 `/users/myos/projects` entries, filters valid project directories and reports the same three fixed source/build/package rows for each; `projstatus <name>` reports those entries through at most 128 VFS-list probes per parent, and `cleanproj <name>` removes only an existing regular fixed build output (`main.elf`) after revalidation; `rmproj <name>` requires that build absent, accepts only the two known project entries, removes a regular source when present and then its empty fixed project directory while retaining the installed package; `editproj`, `buildproj` and `installproj` delegate to the established `edit`, `asm` and `install` programs, preserve their error behavior and retain the installer’s explicit package-replacement behavior. The commands use existing VFS requests, keep source and installed package untouched during cleanproj, and do not claim crash-transactional persistence. The kernel loader recognizes the same exact bounded project output path and still rejects other `/users/...` paths. BIOS regression checks exact default and selected-starter template bytes, duplicate, unknown-starter and trailing-token rejection without creation, empty-starter shell creation-to-editor save/build/run/cleanup, default-starter shell creation-to-build/run/cleanup, creation-to-run/cleanup and creation-to-install/package-run/cleanup, source-ready/build-package-missing status, shortcut editor entry, direct GUI creation-to-build/run/clean/remove, creation-to-install/package-run/uninstall/clean/remove and existing-project build/install/uninstall/clean/remove with native argument forwarding, package readiness, source/build/package preservation, clean-workspace rejection and missing-output/package GUI rejection, shell direct-run/install, ready build/package status, read-only multi-project overview, uninstall/reinstall source-build preservation, clean/rebuild/direct-run behavior, package preservation and execution, then project-directory removal with preserved package; UEFI checks direct GUI build/run/install/uninstall/reinstall/clean plus persisted direct workspace removal, then confirms removed selected source/build, retained package and final missing build outputs.

### Project Hub action mapping

`startgui projects` and the `/users/myos/projects` directory entry from FILES render **Project Hub** through the existing `MYOS_GUI_CONTENT_FLAG_BROWSER` surface. It scans at most 128 fixed-root entries, filters only valid project directories, retains at most four visible entries plus one next-page probe, and uses existing browser action IDs for Refresh, Previous, entries, Next, Status, and Project root. Selecting an entry builds only the exact fixed project path and revalidates its directory before activating Project Workbench. No Project Hub action creates a project, accepts an arbitrary path, or changes a VFS request, syscall, loader rule, or GUI ABI.

### Project Workbench action mapping

Bare `startgui project <name>` renders **Project Workbench** through the existing `MYOS_GUI_CONTENT_FLAG_BROWSER` surface. Its visible content rows 1–9 deliberately match the fixed kernel browser-action mapping: `PARENT`/Refresh, `PREVIOUS`/Edit source, entries 0–3/Build–Uninstall, `NEXT`/Clean build, `CREATE`/Remove workspace, and `CREATE_DIRECTORY`/Status. The state rows begin only after those actions. The Workbench constructs and revalidates only the existing fixed source, build, and package paths; Build, Run (with empty arguments), and Install use their pre-existing console handoff. A successful Workbench removal clears Workbench mode and returns to the `/users/myos/projects` browser, avoiding a stale action surface. This is a user-surface integration only: it adds no VFS request, syscall, loader allowance, or general path grammar.
