...
| `make regression` | Creates a disposable raw-image copy; validates BIOS default `startgui` navigation through M/H/N/H/Q and clean return, then QMP-injected PS/2 mouse activation of the centered `NOTES` tile, `SYSTEM`/`MONITOR` close controls, MONITOR title-bar raise, viewer close-to-home and editor cancel-to-viewer with PPM framebuffer transitions in BIOS and UEFI. It also retains the `startgui home` alias, validates GUI note edit/save, a paced editor-authored 305-byte SDK `cp` copy across the 256-byte VFS boundary, exact readback and no-overwrite behavior, console-editor source persistence, legacy native branches, empty and forwarded `args` output, `input` exact-match/fallback behavior, RTC `HH:MM:SS` output and rejection cases. |
...
| `make regression` | Disposable-image BIOS default `startgui` navigation through M/H/N/H/Q and clean return, QMP-injected PS/2 mouse activation of the centered NOTES tile, SYSTEM/MONITOR close controls, MONITOR title-bar raise, viewer close-to-home and editor cancel-to-viewer with PPM framebuffer transitions pass; the retained `startgui home` alias, GUI note editing, a paced editor-authored 305-byte SDK `cp` copy across the VFS chunk boundary, exact readback and overwrite rejection, editor source workflow, legacy zero/nonzero branches, empty and forwarded native `args`, native `input` exact-match/fallback paths, valid RTC `HH:MM:SS` output and rejected targets also pass. UEFI repeats the full GUI mouse surface, desktop-home navigation, persisted text/copied-target readback, package execution and clean GUI return. |
