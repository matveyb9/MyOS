...
| GUI File Workspace | `startgui` → `FILES` begins at `/users/myos/`, shows the complete current logical path in its bounded title field, may enumerate and traverse every logical VFS directory up to `/`, and does not expose raw boot partitions. |
| GUI mutation boundary | The 16 KiB GUI editor opens only selected existing regular files under `/users/myos/`, `/temp/`, `/system/data/` or `/system/config/`; it does not make `/system/core/`, `/system/live/` or `/apps/` writable. |

### File Workspace v1

The GUI file manager is a ring-3 navigation client, not another filesystem backend. It displays a bounded path tail in content, the complete current path in the window title, a parent row, paging controls and four VFS-enumerated rows. Before entering a directory or opening a file, ring 3 repeats enumeration for the clicked slot and builds a child path only from a printable entry name without `/`. This gives the user free read-only traversal of the visible hierarchy while preserving the VFS type and write policy. In established writable roots it can create an absent empty file or directory, delete a named file or empty directory after a second Enter, copy one regular file of at most 64 KiB to an absent same-directory target, rename one mutable object in place, and move one regular file by metadata update to an existing writable directory without overwrite. MOVE stays within one persistent move anchor or the `/temp` hierarchy; it does not move directories or implement copy-delete. `[SEARCH]` is a separate bounded read-only current-directory operation. Package installation and raw-device operations remain shell-only, while direct `cp` remains the shell interface for arbitrary-path copy workflows.
