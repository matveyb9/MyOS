...
| `startgui project <project-name> new [hello\|args]` | Принимает только bounded absent project name и fixed default starter `hello` либо `args`, создаёт только `<project>/main.mya`, отклоняет existing target и удаляет только own partial creation state после later ordinary failure. Он остаётся в GUI с narrow result status и не создаёт child. |
