...
| `runproj <project-name>` | Повторно проверяет и запускает только regular generated `<project>/main.elf` через existing foreground loader без создания или замены package. При missing build сообщает, что сначала нужен `buildproj`. |
...
| Project direct-run lifecycle | BIOS доказывает, что `buildproj` → `runproj` запускает uninstalled generated template, затем `installproj` создаёт package; после `cleanproj` package всё ещё запускается при missing build, затем rebuild снова запускается напрямую перед final clean. UEFI подтверждает persisted source/package и final missing build output. |
