...
> **Статус:** реализовано и проверено в `gui/bringup`. Встроенный `asm` поддерживает вывод текста, ввод одного байта, вывод времени RTC, именованные метки, одно ограниченное condition value и безусловные или условные переходы только вперёд. Это не general-purpose assembler, C compiler и не замена host [MyOS SDK](SDK_RU.md).
...
Эта программа ожидает один значимый байт. Для заглавной `Y` она выводит `yes`, в остальных случаях — `no`, затем выводит время RTC в виде `HH:MM:SS`.
...
Введите `Y` или другой байт, когда программа ожидает ввод. User shell сообщит выбранную строку, корректную строку времени `HH:MM:SS` и exit status `37`. Для output ELF используйте persistent project paths: generated images немного больше 4 KiB, а temporary VFS намеренно мал.
...
`set` генерирует `mov ebx, imm32`. `input` помещает один байт в fixed scratch location через `MYOS_SYS_READ`, перезагружает scratch pointer после syscall boundary, отфильтровывает `CR` и `LF`, затем помещает принятый байт в `EBX`. `time` читает fixed layout `myos_rtc_time` в ту же private scratch area размером 32 bytes и форматирует часы, минуты и секунды в two decimal digits перед одним write syscall.
...
| Executable statements | Не более 64 суммарных `write`, `input`, `time`, `set`, jump и `exit` instructions. |
...
| ELF layout | Один RX `PT_LOAD` по `0x400000` и fixed RW `PT_LOAD` размером 32 bytes по `0x401000` для private syscall scratch data; relocation, libc и dynamic linker отсутствуют. |
...
| UEFI persistence | Установленный input/time package снова запускается после UEFI/OVMF boot; `A` выбирает ожидаемый path, а time line остаётся корректной. |
