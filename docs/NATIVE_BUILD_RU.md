...
| Project starter/direct-run lifecycle | BIOS доказывает exact default-template bytes, duplicate и unknown-template rejection без project creation, затем `buildproj` → `runproj` запускает uninstalled generated template, а starter `args` напрямую передаёт normal native argument string до `installproj`, создающего package; `projlist` сообщает оба starter projects с их fixed three-way status rows, а после `uninstallproj` source/build остаются ready при missing package до восстановления через `installproj`; после `cleanproj` package всё ещё запускается при missing build, затем rebuild снова запускается напрямую перед final clean. UEFI подтверждает persisted multi-project overview, source/package и final missing build outputs. |
