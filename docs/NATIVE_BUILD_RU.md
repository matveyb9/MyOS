...
`newproj` по умолчанию записывает runnable program `Hello from MyOS project`; `newproj <name> args` записывает fixed bracketed starter `args`, а `newproj <name> empty` создаёт editable regular zero-length source без zero-length write request. Добавьте только exact final token `edit`, чтобы создать выбранный starter и затем открыть в established shell editor лишь его новый fixed source; later editor-launch failure сохраняет completed project. Starter напрямую выводит `[hello MyOS]` через `runproj native-args hello MyOS` и `run native-args hello MyOS`, а без parameters выводит `[]`. Замените любой source через `edit`, чтобы написать другую program; например добавьте `time` перед final `exit 37`, чтобы вывести корректную строку `HH:MM:SS` и вернуть status `37`. Для output ELF используйте persistent project paths: generated images немного больше 4 KiB, а temporary VFS намеренно мал.

## Команды shell

| Команда | Назначение |
|---|---|
| `newproj <project-name> [hello\|args\|empty] [edit]` | Создаёт `/users/myos/projects/<project-name>/main.mya` из одного fixed starter: `hello` используется по умолчанию как runnable template, `args` записывает bracketed native-argument program, а `empty` создаёт editable regular zero-length source без zero-length write request. Единственный optional final token — exact `edit`: только после successful creation он запускает established editor с этим новым fixed source. Names — 1–31 ASCII letters, digits, `-` или `_`; existing project, unknown starter или trailing token отклоняется без создания либо overwrite. Later editor-launch failure сохраняет completed project. |
