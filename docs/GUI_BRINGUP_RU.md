...
| Automated `make regression` | Passed: disposable-image harness created/edited/saved a BIOS GUI note, built/installed/ran legacy native packages, checked exact `input` match and fallback paths plus valid RTC `HH:MM:SS` output, rejected invalid forward-only control flow, затем UEFI прочитал persisted files, повторно запустил installed input/time package и корректно entered/exited GUI. See [RELEASE_STABILIZATION_RU.md](RELEASE_STABILIZATION_RU.md). |
