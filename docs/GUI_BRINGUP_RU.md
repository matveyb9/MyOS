...
> **🌐 LANGUAGE / ЯЗЫК:** **🇷🇺 РУССКИЙ** / [🇺🇸 ENGLISH](GUI_BRINGUP.md)
...
| Save и cancel | `Ctrl-S` заменяет выбранный file в `/users/myos/files/notes/`, записывает draft и возвращает к его viewer. `Esc`, `Alt+F4` на focused NOTES или `X` окна `NOTES` отменяет draft и перезагружает ранее сохранённое содержимое. |
