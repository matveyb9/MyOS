...
| File Workspace | `FILES` начинает в `/users/myos/`, поддерживает parent, directory-entry и paged navigation по полной logical VFS без завершения GUI session и различает directories, regular files и virtual entries. Каждая row имеет fixed visible-name column из 12 characters, type marker `D`/`F`/`L`/`V` и current logical-VFS byte size с suffix `B`. Rows `[NEW FILE]`, `[NEW FOLDER]`, `[DELETE]` и `[COPY]` active только в `/users/myos`, `/temp`, `/system/data` и `/system/config`: каждая принимает одно непустое printable ASCII name до 63 bytes без `/`. Первая создаёт new empty file через unified VFS и открывает bounded GUI editor; вторая создаёт new empty directory и обновляет browser на месте. `[DELETE]` отображает и фиксирует полный named target, затем вызывает existing remove operation только после второго `Enter`; `Esc` отменяет действие до mutation. `[COPY]` принимает source и target names в двух prompts, копирует только regular source в absent target same current directory, stream-копирует не более 64 KiB через existing VFS requests по 256 bytes и удаляет только собственный partial target при copy failure. Read-only roots, invalid names, existing creation paths, non-empty deletion directories и non-regular или oversized copy sources выводят status без mutation. |
...
| Editor entry | Tile `EDIT NOTE` открывает bounded editor для default personal note; отсутствующий default path начинает с пустого draft. FILES открывает existing regular file в том же editor, только если его VFS path writable, а `[NEW FILE]` создаёт и открывает новый empty regular file, тогда как `[NEW FOLDER]` создаёт и показывает new empty directory по тому же writable-root rule, а `[DELETE]` удаляет named file или empty directory только после отображения зафиксированного target и второго `Enter`; `[COPY]` stream-копирует existing regular file до 64 KiB в new same-directory target без overwrite. |
...
| File browser | Четыре entries на page | FILES начинает в `/users/myos/`, повторно перечисляет выбранный logical VFS entry до смены directory или открытия, отображает `D`/`F`/`L`/`V`, name column из 12 characters и byte size entry и допускает traversal до `/` без raw boot media, передаёт полный current directory в title окна и раскрывает bounded create-file, create-directory, named-delete и named-copy actions только в existing writable roots. |
