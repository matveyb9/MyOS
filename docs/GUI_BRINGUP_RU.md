...
| `struct myos_gui_content_request` | 4 240 bytes | `length`, `flags`, `cursor`, `viewport`, `title[112]`, `data[4096]`; копируется только active GUI-session path после отдельной проверки mapped range. Обычный syscall I/O limit 512 bytes не меняется. |
