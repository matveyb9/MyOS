...
| Project Hub и global overview | `startgui projects` постранично показывает только valid project directories fixed project root и открывает selected revalidated entry в Project Workbench; он использует existing browser action surface для refresh, paging, global status и возврата raw project root. `startgui projects status` остаётся read-only: он сканирует до 128 root entries, фильтрует только valid project directories и выводит лишь fixed source/build/package rows без mutation либо child process. |
| Direct project creation | `startgui project <name> new [hello|args|empty]` принимает только bounded name и fixed starter selection, включая editable zero-length source `empty`, создаёт только fixed workspace/source pair, отклоняет existing target и откатывает только own partial creation state. GUI остаётся active с narrow result status и не создаёт child process. |
...
Bare `startgui project <name>` теперь открывает **Project Workbench**, а не generic project file listing. Он сохраняет existing validation project name из 1–31 ASCII characters и показывает fixed state `source`, `build` и `package` только для `/users/myos/projects/<name>/main.mya`, `/users/myos/projects/<name>/main.elf` и `/apps/<name>/main.elf`.

Bounded clickable actions: **Refresh**, **Edit source**, **Build**, **Run**, **Install**, **Uninstall**, **Clean build**, **Remove workspace** и **Status**. Они переиспользуют уже established editor, assembler, loader, installer и one-path removal helpers. Build, run и install передают выполнение existing console child; run из Workbench не принимает free-form tail, тогда как direct form `startgui project <name> run [arguments]` сохраняет bounded native argument tail из 127 visible bytes. Workbench не добавляет VFS primitive, arbitrary path, executable path или loader allowance. Failure action перерисовывает тот же selected workspace с narrow status; successful workspace removal возвращает к browser project root, по-прежнему требует exact clean-workspace condition и сохраняет installed package.
