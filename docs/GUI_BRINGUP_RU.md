...
| Launcher | В desktop-home mode три fixed clickable tiles — `SYSTEM`, `NOTES` и `EDIT NOTE` — и до четырёх обнаруженных package tiles `/apps/<name>/main.elf` заменяют ordinary windows. |
...
| Persistent selection | До 64 scanned directory indices | GUI использует `MYOS_SYS_VFS_LIST` только в notes directory. |
