...
| Focus | Click по title bar открытого ordinary window поднимает его; click по body также сохраняет существующее поведение focus верхнего окна. `Tab`, `Enter` или `Space` вне editor переводят focus на следующее видимое окно. |
