...
`startgui` является обычной ring-3 программой. Без аргументов она открывает bounded home view **MYOS DESKTOP**; `startgui home` остаётся compatibility alias. Его mouse-first launcher предоставляет кликабельные tiles `SYSTEM`, `NOTES` и `EDIT NOTE`, а также top-bar control `X` для выхода. В viewer или editor mode активная поверхность `NOTES` поднимается над статическими окнами; click по title bar любого открытого окна поднимает его. У каждого окна есть собственный `X`: `SYSTEM` и `MONITOR` скрываются, viewer `NOTES` возвращается home, а editor `NOTES` отменяет несохранённый draft и восстанавливает viewer выбранного файла. Отдельный top-bar `X` по-прежнему завершает GUI session. GUI-level keyboard fallback намеренно компактный: `Alt+Tab` переводит focus на следующее видимое окно, `Ctrl+B` возвращает к MYOS DESKTOP, а `Alt+F4` выходит в тот же user shell. Остальные действия launcher и окон выполняются только мышью. Editor сохраняет стандартные `Ctrl+S` для save и `Esc` для cancel; `Ctrl+B` или `Alt+F4` из editor явно отбрасывают несохранённый draft перед возвратом home либо выходом. Она создаёт ограниченную GUI session через существующую syscall boundary, читает и редактирует только bounded user-space payload, а kernel получает лишь проверенные syscall requests. `startgui /users/myos/files/notes/<name>` открывает read-only viewer выбранной persistent note; для default personal note используйте tile `EDIT NOTE`, а для обычных text files — общий `edit <absolute-file>`.
...
| Editor entry | Tile `EDIT NOTE` открывает bounded editor для default personal note; отсутствующий default path начинает с пустого draft. |
...
| Save и cancel | `Ctrl-S` заменяет выбранный file в `/users/myos/files/notes/`, записывает draft и возвращает к его viewer. `Esc` или `X` окна `NOTES` отменяет draft и перезагружает ранее сохранённое содержимое. |
| Built-in choices | Click по `SYSTEM`, `NOTES` или `EDIT NOTE` запускает fixed bounded actions. `NOTES` открывает bounded personal-notes route, а `EDIT NOTE` — editor default personal note. |
...
| Keyboard fallback | `Alt+Tab` переключает focus, `Ctrl+B` возвращает home, а `Alt+F4` выходит. Temporary single-letter GUI commands, keyboard pointer movement, numeric visibility toggles и layout reset удалены. |
| Visibility | Click по per-window `X` скрывает `SYSTEM` или `MONITOR`; `X` окна `NOTES` возвращает viewer home либо отменяет editor. |
| Reset и выход | `Ctrl+B` возвращает home. `Alt+F4` или top-bar `X` завершает session и возвращает framebuffer text console; `Esc` остаётся только отменой editor. |

В editor обычные printable keys становятся текстом draft и не передаются window manager. `Ctrl+S` сохраняет, `Esc` отменяет к viewer, а явно глобальные пути `Ctrl+B`/`Alt+F4` отбрасывают draft перед возвратом home либо выходом.
...
| Input | Existing scheduler-safe console input path; PS/2 auxiliary port выдаёт three-byte packets через IRQ12. PS/2 arrows, Home, End и Delete переводятся в internal bounded key bytes. Keyboard decoder также отображает `Alt+Tab`, `Ctrl+B` и `Alt+F4` в bounded GUI tokens; в editor normal keys принадлежат draft. |
