...
| `runproj <project-name>` | Revalidates and runs only the regular generated `<project>/main.elf` through the existing foreground loader, without creating or replacing a package. A missing build reports that `buildproj` must run first. |
...
| Project direct-run lifecycle | BIOS proves `buildproj` → `runproj` executes the uninstalled generated template, then `installproj` packages it; after `cleanproj`, the package still runs while the build is missing, then a rebuild runs directly again before final clean. UEFI confirms persisted source/package with the final build output missing. |
