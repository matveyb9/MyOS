...
| Project starter/direct-run lifecycle | BIOS proves exact default-template bytes, duplicate and unknown-template rejection without project creation, then `buildproj` → `runproj` executes an uninstalled generated template and the `args` starter forwards its normal native argument string directly before `installproj` packages it; `projlist` reports both starter projects with their fixed three-way status rows, and after `uninstallproj`, source/build remain ready while the package is missing until `installproj` restores it; after `cleanproj`, the package still runs while the build is missing, then a rebuild runs directly again before final clean. UEFI confirms the persisted multi-project overview, source/package and final build outputs missing. |
