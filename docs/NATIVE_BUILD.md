...
| Executable statements | At most 64 total `write`, `args`, `input`, `time`, `set`, `add`, `sub`, `mul`, `div`, `store`, `load`, jump and `exit` instructions. |
...
| Condition | One `0..255` value in generated `EBX`, initialized by `input`, `set` or `load`; `add`, `sub` and `mul` update its low byte modulo 256, while `div` uses an unsigned quotient with a required `1..255` divisor. There is no general user-addressable mutable memory. |
...
These bounds keep parsing, target resolution, code generation and storage static and auditable. Invalid syntax, duplicate labels, a slot outside `0..7`, an add/sub/mul operand outside `0..255`, a divisor outside `1..255`, arithmetic or a conditional jump without `input`, `set` or `load`, missing or non-forward targets, malformed paths or output-write failures leave the shell usable and return a non-zero status.
