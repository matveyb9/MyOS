...
| Executable statements | At most 64 total `write`, `args`, `input`, `time`, `set`, `not`, `neg`, `inc`, `and`, `or`, `xor`, `shl`, `shr`, `rol`, `ror`, `add`, `sub`, `mul`, `div`, `mod`, `store`, `load`, `cmp`, jump and `exit` instructions. |
...
```text
asm: syntax error; set/load/input must precede not/neg/inc/add/sub/mul/div/mod/and/or/xor/shl/shr/rol/ror/cmp and conditional jumps, add/sub/mul/and/or/xor are byte values 0..255, shl/shr/rol/ror are 1..7, div/mod are 1..255, store/load/cmp slots are 0..7, labels need ':' and jumps must target a later label
```
