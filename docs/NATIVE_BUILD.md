...
Start a project with the default fixed `hello` skeleton or the fixed `args` skeleton, then build and run it through the project shortcuts.
...
# Or build the same fixed source/output pair from the GUI entry:
...
`newproj` defaults to a runnable `Hello from MyOS project` program; `newproj <name> args` writes the fixed bracketed `args` starter instead. The starter directly reports `[hello MyOS]` through `runproj native-args hello MyOS` and `run native-args hello MyOS`, while no parameters report `[]`. Replace either source through `edit` to author a different program; for example, add `time` before its final `exit 37` to render a valid `HH:MM:SS` line and return status `37`. Use persistent project paths for output ELF files: generated images are slightly larger than 4 KiB, while the temporary VFS is intentionally small.

## Shell commands

| Command | Purpose |
|---|---|
| `newproj <project-name> [hello|args]` | Creates `/users/myos/projects/<project-name>/main.mya` from one fixed runnable starter: `hello` is the default, while `args` writes a bracketed native-argument program. Names are 1–31 ASCII letters, digits, `-` or `_`; an existing project or unknown template is rejected without creating or overwriting anything. |
