...
| Project starter/direct-run lifecycle | BIOS proves exact default-template bytes, duplicate and unknown-template rejection without project creation, then `buildproj` → `runproj` executes an uninstalled generated template and the `args` starter forwards its normal native argument string directly before `installproj` packages it; after `cleanproj`, the package still runs while the build is missing, then a rebuild runs directly again before final clean. UEFI confirms persisted source/package with the final build output missing. |
