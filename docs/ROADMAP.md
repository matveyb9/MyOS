...
> **Status as of 21 August 2026.** MyOS is an in-house educational and practical OS for `x86_64`, written in freestanding C11 and x86_64 NASM. The project is not based on Linux or BSD; Limine is used only as the current bootloader and provider of the boot environment. The stable console line is complete, and the new GUI functionality is isolated in a separate branch.
...
| `[x]` | User utilities | `hello`, `sleeper`, `orphaner`, `safety`, `argshow`, `calc`, `pipewrite`, `piperead`, `wc`, `grep`, `edit`, `tree`, `find`, `head`, `tail`, `sort`, `stat`. `tree` and `find` are read-only bounded logical-VFS explorer/search utilities with fixed recursion/output limits; `head` reads the beginning of one absolute file in 256-byte chunks with a 1–64 line request and 4 KiB output cap; `tail` streams one absolute file while retaining only its final 4 KiB for a 1–64 line request; `sort` stably orders up to 64 retained 127-byte lines in bytewise ASCII order; `stat` reports type and size through a bounded parent-directory scan. |
...
| `[x]` | File Workspace v1 | `FILES` begins at `/users/myos/`, browses the complete logical VFS through four-entry pages, displays bounded `D`/`F`/`L`/`V` type markers, fixed 12-character names and byte sizes, and opens writable regular files in the GUI editor without changing VFS write policy. |
...
| `[x]` | Desktop clock and task status | The compositor draws a normalized RTC `HH:MM:SS` clock and bounded `TASKS`/`RUN` scheduler snapshots without a new ABI or background repaint loop; BIOS/UEFI QEMU captures require both text regions. |
...
| 15 | `[x]` | File Workspace v1 | Compact `FILES` starts at `/users/myos/`, freely traverses the logical VFS through four-entry pages, shows `D`/`F`/`L`/`V`, fixed 12-character names and byte sizes, safely views system/inventory records and invokes the 16 KiB GUI editor only for writable roots. BIOS/UEFI QMP regression covers tile entry, parent navigation, visible metadata and CPIO-backed `/system/core/apps` traversal. |
| 16 | `[x]` | Initial GUI editor 4 KiB expansion | The initial fixed GUI content ABI exposed 4,096-byte viewer/editor payloads through a dedicated mapped-range copy while ordinary syscall I/O remained capped at 512 bytes. This historical step was superseded by milestone 24. |
| 17 | `[x]` | Bounded native variables | `.mya` now supports `store <0..7>` and `load <0..7>` over eight zero-initialized private byte slots in the fixed 32-byte RW segment. BIOS builds, installs and runs a value-preserving exact branch, rejects slot `8`, and UEFI reruns the persisted package. |
| 18 | `[x]` | SDK VFS write reference | `sdk/examples/write.c` is staged as `/system/core/examples/sdk/write.elf`; after explicit installation it creates one new absolute target and writes a fixed payload through public VFS wrappers. BIOS verifies exact payload and no overwrite, while UEFI validates the persisted package and a second exact target. |
| 19 | `[x]` | Live desktop clock | PIT IRQ0 updates the compositor-owned `HH:MM:SS` once per second by repainting only its 72×32 top-bar rectangle and preserving the pointer; windows, tiles and footer are not redrawn. BIOS and UEFI QEMU captures prove that an unchanged launcher clock region advances. |
| 20 | `[x]` | Native byte arithmetic | `.mya` supports `add <0..255>` and `sub <0..255>` over the initialized accumulator with modulo-256 wrap; BIOS validates `(250 + 8 - 2) mod 256 = 0` branch and UEFI reruns the persisted package. |
| 21 | `[x]` | Bounded native multiply/divide | `.mya` supports `mul <0..255>` with modulo-256 low-byte retention and safe unsigned `div <1..255>`; BIOS validates `((200 * 2 mod 256) + 57) / 3 = 67`, while `div 0` is rejected and UEFI reruns the persisted `MULDIV` package. |
| 22 | `[x]` | Bounded private comparison | `.mya` supports `cmp <0..7>` over the initialized accumulator and an eight-slot private byte store; equality produces `0` and inequality `1` for existing forward-only branches. BIOS validates `EQ` and `NE`, rejects uninitialized or slot-`8` comparisons, and UEFI reruns the persisted package. |
| 23 | `[x]` | File Workspace full-path title | `FILES` now passes the complete current logical-VFS directory through the bounded 112-byte GUI title field; compact title glyph spacing displays the whole path at the supported 1280×800 geometry. BIOS QMP captures validate visible title text and title transitions after parent and `/system` navigation; UEFI repeats the File Workspace workflow. |
| 24 | `[x]` | 16 KiB GUI editor documents | The fixed GUI content ABI now carries 16,384-byte viewer/editor documents in a 16,528-byte mapped request. Ring 3 reads and saves through up to sixty-four unchanged 256-byte VFS chunks; BIOS copies a deterministic 16 KiB fixture, saves/reloads it in GUI and verifies exact content, while UEFI confirms the persisted payload. |
| 25 | `[x]` | Direct shell copy | `cp <absolute-source> <new-absolute-target>` now invokes the existing bounded SDK-built copy app without lifecycle noise and appears in shell completion/help. It retains 256-byte transfers, an absent-target/existing-parent rule and partial-target cleanup; `run cp` remains compatible. BIOS verifies exact 305-byte copy plus both direct and compatibility overwrite rejection, while UEFI repeats the direct rejection against the persisted target. |
| 26 | `[x]` | Direct shell word count | `wc <absolute-file>` now invokes the existing bounded stream counter directly, reports newline lines, space/tab/CR/LF-delimited words and bytes, and appears in shell completion/help. BIOS validates exact `1 lines, 128 words, 259 bytes` output where the final word crosses the 256-byte VFS chunk boundary, while UEFI repeats the persisted direct count; `run wc` remains compatible. |
| 27 | `[x]` | Direct shell text search | `grep <text> <absolute-file>` now invokes the bounded native search directly and appears in shell completion/help. It prints matching newline-terminated lines up to 127 bytes and skips overlong lines while reading 256-byte VFS chunks. BIOS verifies one short match while a matching overlong line is skipped; UEFI repeats the persisted direct search; `run grep` remains compatible. |
| 28 | `[x]` | File Workspace empty-file creation | FILES now exposes a bounded `[NEW FILE]` browser row. In `/users/myos`, `/temp`, `/system/data` or `/system/config`, it accepts a printable slash-free name up to 63 bytes, creates only a new empty VFS file and opens the existing GUI editor; read-only roots remain immutable. BIOS QMP creates and saves `/users/myos/guinew`, while UEFI confirms its persisted zero-byte record. |
| 29 | `[x]` | Native bitwise byte operations | The in-OS assembler now accepts operand-free `not` and bounded `and <0..255>` / `or <0..255>` on the initialized byte accumulator. BIOS builds a `BITWISE` package that computes `~240 & 63 | 128 = 143`, while UEFI repeats the persisted branch; uninitialized `not` and `and 256` are rejected. |
| 30 | `[x]` | Native exclusive-or byte operation | The in-OS assembler now accepts bounded `xor <0..255>` on the initialized byte accumulator. BIOS builds an `XOR` package that computes `170 xor 255 xor 85 = 0`, while UEFI repeats the persisted zero branch; uninitialized `xor` and `xor 256` are rejected. |
| 31 | `[x]` | Direct shell stat | `stat <absolute-path>` now invokes the existing bounded metadata lookup directly, reports logical entry type and byte size, and appears in completion/help. BIOS verifies direct help, exact MOTD type/size and retained `run stat` compatibility; UEFI repeats direct MOTD and persisted zero-byte records. |
| 32 | `[x]` | Direct shell sort | `sort <absolute-file>` now invokes the existing bounded text-ordering tool directly, emits up to 64 retained 127-byte lines in bytewise ASCII ascending order, and appears in completion/help. BIOS verifies direct help, exact MOTD order and retained `run sort` compatibility; UEFI repeats direct ordering. |
...
Native build workflow, labels/forward-only jumps, bounded conditional control flow, SDK VFS copy tooling, direct bounded word counting and text search, native input/time, the bounded default `startgui` desktop launcher with dynamic installed-app tiles, the general text editor, GUI pointer-refresh hardening, the read-only System Inventory VFS with `sysinfo`, and the automated release-stabilization baseline are completed in `feature/gui`: MYPFS004 VFS provides a unified root with `/system`, `/apps`, `/users/myos` and `/temp`, and `build` compiles bounded `.mya` source into a native ELF64. `args` forwards the existing bounded `run <name> [arguments]` string. Eight private `store`/`load` slots retain program bytes, but general user-addressable writable memory is not introduced. `input` captures one non-`CR`/`LF` byte as the condition; `set <0..255>` supplies an explicit alternative; `not`, `and <0..255>`, `or <0..255>`, `xor <0..255>`, `add <0..255>`, `sub <0..255>` and `mul <0..255>` require that initialized byte accumulator; bitwise operations update its eight bits and arithmetic wraps modulo 256, while `div <1..255>` rejects zero and retains an unsigned integer quotient; `cmp <0..7>` compares that accumulator with private state and yields `0` for equality or `1` for inequality; `jump_if_zero`, `jump_if_nonzero` and `jump_if <0..255>` keep every target strictly forward. `time` outputs one RTC line in `HH:MM:SS` format. A fixed 32-byte private RW ELF segment retains the entry argument pointer, syscall scratch storage and eight private slots, not general writable program data. The SDK header also exposes fixed-size VFS read/create/write/remove wrappers; direct shell `cp` invokes its live tool to copy files in 256-byte chunks, requires a new destination with an existing parent and never overwrites it, while `run cp` remains compatible. Direct `wc` streams line/word/byte counts through 256-byte VFS chunks, direct `grep` prints matching lines up to 127 bytes and skips longer lines, direct `sort` emits bounded bytewise ASCII order, and direct `stat` reports bounded logical-entry type/size; `run wc`, `run grep`, `run sort` and `run stat` remain compatible. Bare `startgui` is a bounded mouse-first launcher: its four compact system-tile rectangles (`SYSTEM`, `NOTES`, `EDIT NOTE`, `FILES`) and top-bar exit rectangle dispatch existing actions; FILES starts at `/users/myos/`, sends the full current logical path through its 112-byte title field, re-enumerates bounded VFS rows before navigation, reaches `/` without exposing raw boot media, creates new empty files only in its established writable roots from a 63-byte slash-free prompt and opens only writable files up to 16 KiB in its GUI editor, while up to four verified `/apps/<name>/main.elf` packages appear below as mouse-only launch tiles. A tile revalidates and spawns its exact package, ends GUI and waits for the child so console output remains visible. `Alt+Tab` cycles focus, `Alt+F4` closes the focused window, `Esc` returns or cancels, and `Ctrl+Q` exits; temporary single-letter, pointer, numeric and reset controls are removed. `startgui home` remains an alias rather than a general window API or application installer. Non-launcher content raises the active NOTES surface; exposed window title bars raise their records, and per-window `X` controls hide SYSTEM/MONITOR, return viewer NOTES home, or cancel editor NOTES without saving. Direct `edit <absolute-file>` provides cursor-based multi-line editing for ordinary files and `.mya` source, with explicit save/discard and a 4 KiB in-memory document limit. The disposable-image `make regression` gate covers QMP PS/2 `Alt+Tab` focus, `Alt+F4` focused-MONITOR close, `Esc` viewer return, `Alt+F4` editor cancel-to-viewer and `Ctrl+Q` clean exit in BIOS and UEFI, plus mouse activation of compact `NOTES` and `FILES` tiles, visible File Workspace title transitions for parent and `/system` navigation, FILES parent navigation, SYSTEM/MONITOR close, MONITOR title-bar raise, viewer close-to-home, editor cancel-to-viewer and a discovered installed-app tile launch with PPM framebuffer transitions; it also covers the retained `startgui home` alias, the System Inventory directory tree and `sysinfo` output in BIOS and UEFI, ordinary-text readback, a 16 KiB GUI editor load/save/readback through sixty-four VFS chunks seeded from a deterministic initramfs fixture, File Workspace mouse creation and GUI-editor save of zero-byte `/users/myos/guinew` with UEFI type/size persistence, a paced 305-byte direct shell `cp` copy across the VFS request boundary with direct and compatibility overwrite rejection, exact direct `wc` line/word/byte output across a 256-byte boundary with `run wc` compatibility, direct `grep` short-match output with a matching overlong line skipped and `run grep` compatibility, editor-authored program build/run, empty and forwarded arguments, legacy and exact input branches, modular `(250 + 8 - 2) mod 256` add/sub arithmetic with rejected uninitialized `add`, persisted `MULDIV` multiply/divide arithmetic with rejected `div 0`, persisted `BITWISE` not/and/or byte operations from `240` to `143` with rejected uninitialized `not` and `and 256`, persisted `XOR` byte operation `170 xor 255 xor 85 = 0` with rejected uninitialized `xor` and `xor 256`, persisted `EQ`/`NE` private-slot comparison with rejected uninitialized or slot-`8` `cmp`, valid RTC time output, invalid-control-flow rejection, and UEFI persistence of the copied file and installed native packages.
