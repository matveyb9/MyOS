...
| `[x]` | Keyboard interaction | Compact modifier fallback: `Alt+Tab` focus, `Ctrl+B` desktop return and `Alt+F4` exit; editor retains `Ctrl+S` and `Esc`. |
...
| 13 | `[x]` | Standard modifier hotkeys | Retired the temporary `M`/`N`/`E`/`H`/`Q`, pointer, numeric and reset letter controls. `Alt+Tab` cycles focus, `Ctrl+B` returns home and `Alt+F4` exits; `Ctrl+S`/`Esc` remain editor controls. QMP BIOS/UEFI regression injects all three GUI-level modifier shortcuts. |
