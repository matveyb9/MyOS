...
| 15 | `[x]` | File Workspace v1 | Compact `FILES` starts at `/users/myos/`, freely traverses the logical VFS through four-entry pages, safely views system/inventory records and invokes the existing 128-byte GUI editor only for writable roots. BIOS/UEFI QMP regression covers tile entry and parent navigation. |
