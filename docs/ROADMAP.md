...
| 15 | `[x]` | File Workspace v1 | Compact `FILES` starts at `/users/myos/`, freely traverses the logical VFS through four-entry pages, safely views system/inventory records and invokes the 1 KiB GUI editor only for writable roots. BIOS/UEFI QMP regression covers tile entry, parent navigation and CPIO-backed `/system/core/apps` traversal. |
| 16 | `[x]` | GUI editor 1 KiB expansion | The fixed GUI content ABI now supports 1,024-byte viewer/editor payloads. The active GUI-session syscall uses a dedicated mapped-range copy while ordinary syscall I/O remains capped at 512 bytes; ring 3 uses at most four 256-byte VFS reads or writes. BIOS verifies a GUI 1 KiB save/readback and UEFI verifies exact persisted content. |
