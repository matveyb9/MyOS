...
| 15 | `[x]` | File Workspace v1 | Compact `FILES` starts at `/users/myos/`, freely traverses the logical VFS through four-entry pages, shows `D`/`F`/`L`/`V`, fixed 12-character names and byte sizes, safely views system/inventory records and invokes the 4 KiB GUI editor only for writable roots. BIOS/UEFI QMP regression covers tile entry, parent navigation, visible metadata and CPIO-backed `/system/core/apps` traversal. |
