...
| 11 | `[x]` | Bounded desktop-home launcher | `startgui home` presents the fixed keyboard launcher `MYOS DESKTOP`: `M` opens the system message, `N` opens notes, `E` opens the selected-note editor, `H` returns home, and `Q` returns to the shell. It uses the existing single GUI session and bounded viewer/editor state. `make regression` verifies M/H/N/H/Q navigation and clean return in BIOS and UEFI. [GUI_BRINGUP.md](GUI_BRINGUP.md) records the contract. |
