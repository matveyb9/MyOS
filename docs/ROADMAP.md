...
| `gui/bringup` | Isolated development of a framebuffer GUI and user-program platform. | `[~]` Preview checkpoint `v0.12.2-gui-preview` and GitHub Pre-release `v0.13.0-gui-rc.1` published; MYPFS004, persistent ELF execution, external SDK and restricted native toolchain are being developed without merging into `main`. |
...
| 7 | `[ ]` | Next expansion of the native toolchain | Limited conditional jumps based on explicit compare/set operations, then additional syscalls, a multi-line editor and a small linker. After stabilizing these steps, a more complete C subset, a basic C library and build scripts can be evaluated. GCC/Clang are not the first step. |
...
Native build workflow, labels/forward-only jumps, GUI pointer-refresh hardening and automated release-stabilization baseline are completed in `gui/bringup`: MYPFS004 VFS provides a unified root with `/system`, `/apps`, `/users/myos` and `/temp`, and `build` compiles bounded `.mya` source from `/users/myos/projects/` into a native ELF64. `label name:` defines a bounded target, and `jump name` skips code only toward the next label; BIOS build/install/run, backward-jump rejection and UEFI persistence are confirmed. `install` explicitly moves the output to `/apps/<name>/main.elf`, after which `run <name>` creates a separate ring-3 task. `make smoke` confirms raw-image BIOS/UEFI boot markers, `make regression` uses a disposable image to confirm GUI note save, forward-jump native package creation, backward-target rejection and UEFI readback, and `make release-check` produces clean-rebuild source/artifact evidence. The next merge-oriented GUI milestone still requires a physical x86_64 PC smoke test, final release scope and an explicit decision on a new immutable GUI tag. Parallel next native-toolchain work is limited conditional jumps and then convenient multi-line project editing; personal application installation (`/users/myos/apps`) remains a separate future extension. Preview `v0.12.2-gui-preview` and Pre-release `v0.13.0-gui-rc.1` are not merged automatically into `main`; `main` and `console-stable` retain a console-only scope. The source `myos.iso` and `myos.img` continue to be built by the `make all img` command.
