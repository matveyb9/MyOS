...
| 11 | `[x]` | Bounded mouse-first desktop launcher | Bare `startgui` presents fixed clickable `SYSTEM`, `NOTES` and `EDIT NOTE` tiles plus top-bar `X` exit; `startgui home` is a compatibility alias. `M`/`N`/`E`/`H`/`Q` remain hotkey fallback. The launcher uses the existing single GUI session and bounded viewer/editor state. `make regression` verifies default M/H/N/H/Q navigation, QMP PS/2 click activation of `NOTES` with a PPM framebuffer transition in BIOS and UEFI, plus the retained BIOS alias. [GUI_BRINGUP.md](GUI_BRINGUP.md) records the contract. |
