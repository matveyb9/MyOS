...
| 20 | `[x]` | Native byte arithmetic | `.mya` now supports `add <0..255>` and `sub <0..255>` over the initialized accumulator with modulo-256 wrap; BIOS validates `(250 + 8 - 2) mod 256 = 0` branch and UEFI reruns the persisted package. |
