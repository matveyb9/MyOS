...
Native build workflow, labels/forward-only jumps, bounded conditional control flow, the general text editor, GUI pointer-refresh hardening and the automated release-stabilization baseline are completed in `gui/bringup`: MYPFS004 VFS provides a unified root with `/system`, `/apps`, `/users/myos` and `/temp`, and `build` compiles bounded `.mya` source into a native ELF64. `set <0..255>` supplies one explicit condition for `jump_if_zero` or `jump_if_nonzero`; every target remains strictly forward. Direct `edit <absolute-file>` provides cursor-based multi-line editing for ordinary files and `.mya` source, with explicit save/discard and a 4 KiB in-memory document limit. BIOS ordinary-text readback, editor-authored program build/run, missing-condition and backward-target rejection, and UEFI persistence are covered by the disposable-image `make regression` gate. `install` explicitly moves an output to `/apps/<name>/main.elf`, after which `run <name>` creates a separate ring-3 task. `make smoke` confirms raw-image BIOS/UEFI boot markers, and `make release-check` produces clean-rebuild source/artifact evidence. The next merge-oriented GUI milestone still requires a physical x86_64 PC smoke test, final release scope and an explicit decision on a new immutable GUI tag. Parallel native-platform work now begins with selected input and time syscalls; personal application installation (`/users/myos/apps`) remains a separate future extension. Preview `v0.12.2-gui-preview` and Pre-release `v0.13.0-gui-rc.1` are not merged automatically into `main`; `main` and `console-stable` retain a console-only scope. The source `myos.iso` and `myos.img` continue to be built by the `make all img` command.
