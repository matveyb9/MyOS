...
In each established writable root, `[NEW FILE]`, `[NEW FOLDER]` and `[DELETE]` accept one non-empty printable slash-free ASCII name of at most 63 bytes. The file action creates one new empty regular file and opens its GUI editor; the folder action creates one new empty directory; `[DELETE]` removes the named file or empty directory. Each successful action refreshes the browser in place. Existing paths, read-only roots, non-empty directories and malformed names are rejected without mutation. Rename, copy/move, package installation and raw-device operations remain shell-only; use `rm`, `cp` and `install` for those workflows.
