...
| `startgui` | `startgui` | Start the experimental framebuffer GUI. Its footer shows the bounded active surface as `FOCUS HOME`, `FOCUS SYSTEM`, `FOCUS NOTES` or `FOCUS MONITOR`. Click `FILES` to browse the logical VFS from `/users/myos/`; its `[NEW FILE]`, `[NEW FOLDER]`, `[DELETE]`, `[COPY]` and `[RENAME]` rows accept a slash-free name up to 63 printable ASCII bytes only in `/users/myos`, `/temp`, `/system/data` or `/system/config`. The first creates a new empty file and opens it in the GUI editor; the second creates a new directory and refreshes the browser; `[DELETE]` shows the named target and requires a second `Enter` before it removes a file or empty directory; `[COPY]` streams an existing regular source no larger than 64 KiB to an absent same-directory target; `[RENAME]` changes the name of an existing mutable file or directory within that same directory without copying its content. Existing writable text files up to 16 KiB also open there. |
