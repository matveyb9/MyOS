...
The program prints `[hello MyOS]`, then writes the current RTC time as `HH:MM:SS` and returns status `37`. Running `run native-args` with no parameters prints `[]`. The source language supports `args`, `input`, `time`, `set <0..255>`, `store <0..7>`, `load <0..7>`, `label name:`, `write "text"`, `jump name`, `jump_if_zero name`, `jump_if_nonzero name`, `jump_if <0..255> name`, and a final `exit <0..255>`. `store` saves the current condition byte in one of eight private slots; `load` restores it as the condition and is valid before a conditional jump. The slots are private to the running program, zero-initialized, cannot be named or addressed directly, and do not provide arithmetic. A conditional jump needs an earlier `input`, `set` or `load`; every target must be a defined label located later in the source, so loops and backward jumps are rejected. Escapes `\n`, `\r`, `\t`, `\\`, and `\"` are available inside text. For example, `set 73; store 2; set 0; load 2; jump_if 73 matched` preserves the value through slot `2`; slot numbers outside `0..7` are rejected. The generated program runs in ring 3 and returns its authored exit status; use `help asm` and `help edit` for concise command help, [Text Editor](TEXT_EDITOR.md) for editing controls, and [Native Build](NATIVE_BUILD.md) for all bounds and syntax rules.
