...
`[SEARCH]` is available in every browsable root, including read-only roots. It accepts a non-empty printable slash-free ASCII name fragment of at most 63 bytes, scans at most 128 entries in the current directory with case-insensitive matching, renders matching logical-VFS metadata without mutation, and returns to the browser on `Esc`.

In each established writable root, `[NEW FILE]`, `[NEW FOLDER]`, `[DELETE]` and `[COPY]` accept non-empty printable slash-free ASCII names of at most 63 bytes. The file action creates one new empty regular file and opens its GUI editor; the folder action creates one new empty directory. `[DELETE]` first displays the complete named target and removes that file or empty directory only after a second **Enter** confirmation; `Esc` cancels before mutation. `[COPY]` prompts for a source regular-file name and an absent target name in the **same current directory**, then streams at most **64 KiB** through existing 256-byte VFS requests; it never overwrites a target and removes its own partial target on failure. Each successful action refreshes the browser in place. Existing paths, read-only roots, non-empty directories, non-regular or oversized copy sources and malformed names are rejected without mutation. Rename, copy/move beyond this same-directory bounded GUI flow, package installation and raw-device operations remain shell-only; use `rm`, `cp` and `install` for those workflows.
...
| `startgui` | `startgui` | Start the experimental framebuffer GUI. Its footer shows the bounded active surface as `FOCUS HOME`, `FOCUS SYSTEM`, `FOCUS NOTES` or `FOCUS MONITOR`. Click `FILES` to browse the logical VFS from `/users/myos/`; its `[NEW FILE]`, `[NEW FOLDER]`, `[DELETE]` and `[COPY]` rows accept a slash-free name up to 63 printable ASCII bytes only in `/users/myos`, `/temp`, `/system/data` or `/system/config`. The first creates a new empty file and opens it in the GUI editor; the second creates a new directory and refreshes the browser; `[DELETE]` shows the named target and requires a second `Enter` before it removes a file or empty directory; `[COPY]` streams an existing regular source no larger than 64 KiB to an absent same-directory target. Existing writable text files up to 16 KiB also open there. |
