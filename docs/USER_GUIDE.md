...
Without an argument `startgui` opens **MYOS DESKTOP**, a bounded keyboard launcher; `startgui home` remains an alias. The desktop launcher: `M` opens the system message, `N` opens notes, `E` edits the selected note, `H` returns to the launcher, and `Q` exits to the shell. For a personal note you can pass an absolute path:
