...
Use `startgui projects` for a bounded, paged list of valid project workspaces. **Refresh**, **Previous**, **Next**, and **Status** reuse the existing browser action surface; clicking a project entry revalidates only that directory and opens its Project Workbench. **Project root** returns to the raw `/users/myos/projects` File Workspace. The Hub neither creates projects nor accepts arbitrary filesystem paths, and `startgui projects status` remains the read-only overview command.

### Project Workbench

For a single project, use bare `startgui project <name>` to open **Project Workbench** instead of browsing its directory. Its visible actions are **Refresh**, **Edit source**, **Build**, **Run**, **Install**, **Uninstall**, **Clean build**, **Remove workspace**, and **Status**, with fixed source/build/package rows. Each action remains bounded to the named project's `main.mya`, `main.elf`, and matching package `/apps/<name>/main.elf`; no arbitrary file, executable, or destination can be selected. **Build**, **Run**, and **Install** close the GUI and report through their existing console child. Workbench **Run** uses empty native arguments; use `startgui project <name> run [arguments]` or `runproj <name> [arguments]` when the bounded 127-visible-byte argument tail is needed. **Remove workspace** still requires a clean workspace and preserves the installed package.
