...
In each established writable root, `[NEW FILE]`, `[NEW FOLDER]` and `[DELETE]` accept one non-empty printable slash-free ASCII name of at most 63 bytes. The file action creates one new empty regular file and opens its GUI editor; the folder action creates one new empty directory. `[DELETE]` first displays the complete named target and removes that file or empty directory only after a second **Enter** confirmation; `Esc` cancels before mutation. Each successful action refreshes the browser in place. Existing paths, read-only roots, non-empty directories and malformed names are rejected without mutation. Rename, copy/move, package installation and raw-device operations remain shell-only; use `rm`, `cp` and `install` for those workflows.
...
| `startgui` | `startgui` | Start the experimental framebuffer GUI. Its footer shows the bounded active surface as `FOCUS HOME`, `FOCUS SYSTEM`, `FOCUS NOTES` or `FOCUS MONITOR`. Click `FILES` to browse the logical VFS from `/users/myos/`; its `[NEW FILE]`, `[NEW FOLDER]` and `[DELETE]` rows accept a slash-free name up to 63 printable ASCII bytes only in `/users/myos`, `/temp`, `/system/data` or `/system/config`. The first creates a new empty file and opens it in the GUI editor; the second creates a new directory and refreshes the browser; `[DELETE]` shows the named target and requires a second `Enter` before it removes a file or empty directory. Existing writable text files up to 16 KiB also open there. |
