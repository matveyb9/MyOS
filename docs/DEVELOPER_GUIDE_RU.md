...
`newproj <name>` — bounded project entry point. Он принимает ровно один component из 1–31 ASCII letters/digits/`-`/`_`, создаёт `/users/myos/projects/<name>/main.mya` из одного fixed runnable template, никогда не перезаписывает existing project и удаляет только собственное partial setup state при последующем ordinary setup failure. `editproj <name>` строит fixed source/editor path (`main.mya`), `buildproj <name>` строит source/output pair (`main.mya` → `main.elf`), а `installproj <name>` — output/package pair (`main.elf` → `/apps/<name>/main.elf`), `projstatus <name>` выводит три fixed source/build/package entries через не более 128 VFS-list probes на parent directory, а `cleanproj <name>` удаляет только existing regular fixed build output (`main.elf`) после revalidation; первые три делегируют established programs `edit`, `asm` и `install`, сохраняют их error behavior и сохраняют explicit package-replacement behavior installer. Команды используют существующие VFS requests, cleanproj сохраняет source и installed package и не заявляют crash-transactional persistence. BIOS regression проверяет template bytes, duplicate rejection, source-ready/build-package-missing status, shortcut editor entry, shortcut build/install, ready build/package status, clean/rebuild behavior, package preservation и execution; UEFI проверяет persisted template, package, package-ready status и final missing build output.
