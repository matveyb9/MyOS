...
| `make regression` | Создаёт disposable raw-image copy; проверяет QMP-injected PS/2 `Alt+Tab` focus, `Ctrl+B` desktop return и `Alt+F4` clean exit в BIOS и UEFI, затем mouse activation centered tile `NOTES`, controls закрытия `SYSTEM`/`MONITOR`, подъём MONITOR по title bar, viewer close-to-home и editor cancel-to-viewer с PPM framebuffer transitions. Он также сохраняет alias `startgui home`, проверяет GUI note edit/save, paced editor-authored 305-byte SDK `cp` copy через 256-byte VFS boundary, exact readback и no-overwrite behavior, console-editor source persistence, legacy native branches, empty и forwarded `args` output, exact-match/fallback behavior инструкции `input`, RTC output `HH:MM:SS` и rejection cases. |
...
COM1 output is mirrored to the framebuffer text console. Keyboard driver обрабатывает PS/2 Set 1 US QWERTY characters, декодирует `Alt+Tab`, `Ctrl+B` и `Alt+F4` в explicit bounded tokens, предоставляет bounded internal character-injection helper для mouse-generated GUI actions и будит tasks в состоянии `INPUT`. `Alt+Tab` намеренно использует low control token, поэтому existing validation GUI syscall `0..127` не расширяется; остальные tokens потребляет непосредственно GUI owner. Framebuffer сопоставляет с fixed launcher или window-chrome hit rectangles только internal mouse-action tokens; второй input queue или general pointer IPC не создаётся. Каждое non-launcher content update поднимает internal record `NOTES` перед redraw, поэтому активный viewer/editor остаётся видим. После bootstrap `kernel/console/shell.c` ждёт три секунды `K` от PS/2 или COM1: без отмены `/init` запускается автоматически; `K` сохраняет diagnostic kernel shell, где `init` всё ещё запускает ту же user shell вручную. Если `/init` недоступен или automatic loading не проходит, kernel сообщает об этом и остаётся в kernel shell без retry loop.
...
| `make regression` | Disposable-image QMP PS/2 `Alt+Tab` focus, `Ctrl+B` desktop return и `Alt+F4` clean exit проходят в BIOS и UEFI; также проходят mouse activation centered tile NOTES, controls закрытия SYSTEM/MONITOR, подъём MONITOR по title bar, viewer close-to-home и editor cancel-to-viewer с PPM framebuffer transitions. Retained alias `startgui home`, GUI note editing, paced editor-authored 305-byte SDK `cp` copy через VFS chunk boundary, exact readback и overwrite rejection, editor source workflow, legacy zero/nonzero branches, empty и forwarded native `args`, native `input` exact-match/fallback paths, valid RTC `HH:MM:SS` output и rejected targets также проходят. UEFI повторяет полную GUI modifier/mouse surface, чтение persisted text/copied target, package execution и clean GUI return. |
