...
| `make regression` | Создаёт disposable raw-image copy; проверяет BIOS default navigation `startgui` через M/H/N/H/Q и clean return, затем QMP-injected PS/2 mouse activation centered tile `NOTES`, controls закрытия `SYSTEM`/`MONITOR`, подъём MONITOR по title bar, viewer close-to-home и editor cancel-to-viewer с PPM framebuffer transitions в BIOS и UEFI. Он также сохраняет alias `startgui home`, проверяет GUI note edit/save, paced editor-authored 305-byte SDK `cp` copy через 256-byte VFS boundary, exact readback и no-overwrite behavior, console-editor source persistence, legacy native branches, empty и forwarded `args` output, exact-match/fallback behavior инструкции `input`, RTC output `HH:MM:SS` и rejection cases. |
...
| `make regression` | Disposable-image BIOS default navigation `startgui` через M/H/N/H/Q и clean return, QMP-injected PS/2 mouse activation centered tile NOTES, controls закрытия SYSTEM/MONITOR, подъём MONITOR по title bar, viewer close-to-home и editor cancel-to-viewer с PPM framebuffer transitions проходят; также проходят retained alias `startgui home`, GUI note editing, paced editor-authored 305-byte SDK `cp` copy через VFS chunk boundary, exact readback и overwrite rejection, editor source workflow, legacy zero/nonzero branches, empty и forwarded native `args`, native `input` exact-match/fallback paths, valid RTC `HH:MM:SS` output и rejected targets. UEFI повторяет полную GUI mouse surface, desktop-home navigation, чтение persisted text/copied target, package execution и clean GUI return. |
