...
| `main` | Экспериментальная QEMU-validated integration branch для GUI и user-program work. Каждое новое функциональное изменение создаётся в короткоживущей ветке от этого ref и сливается только после проверенной QEMU validation. |
...
Обычная публикация source может push-ить `main` и `console-stable`. Пока не изменится policy physical-PC validation, каждый публичный artifact из `main` — отдельно подтверждённый **Pre-release**, но не stable Release. Build artifacts `myos.iso` и `myos.img` — release attachments, а не Git-tracked source files.
