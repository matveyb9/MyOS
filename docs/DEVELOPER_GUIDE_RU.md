...
Этот документ описывает текущую QEMU-validated integration line **`main`** MyOS. Он предназначен для разработчиков, системных программистов и читателей, которым нужна карта исходного кода и технические инварианты. Стабильный console baseline остаётся в `console-stable` на immutable теге `v0.12.1-console`; GUI, SDK и MYPFS004 changes в `main` не меняют эту стабильную линию без отдельного maintenance decision.

> `main` — экспериментальная QEMU-validated integration line для GUI и user-program platform. Новая функциональная работа развивается в короткоживущей feature-ветке от `main` и возвращается только после проверенного QEMU validation gate. `console-stable` остаётся отдельным стабильным console baseline.
...
The release includes write/read, process lifecycle, task info, VFS read/enumeration/create/write/remove/rename/move, RTC/uptime, bounded spawn arguments, tmpfs/persistent-file operations, pipe operations, reboot and poweroff. Executable loader allowlist принимает `/system/core/apps/...`, installed packages `/apps/<name>/main.elf` и только exact bounded development output `/users/myos/projects/<name>/main.elf`, где `<name>` содержит 1–31 ASCII letters, digits, `-` или `_`; он отклоняет каждый иной user-controlled path. The most important limits are:
...
`MYOS_SYS_VFS_RENAME` использует один fixed `myos_vfs_rename_request` с independently NUL-terminated absolute paths `source` и `target`. Dispatcher validates оба complete request до resolution любого пути. VFS принимает только mutable persistent user object или tmpfs object, когда source и target разрешаются в **one same parent**; roots, fixed hierarchy records, read-only objects, missing source, malformed paths и target, называющий другой existing object, отклоняются. Successful persistent rename записывает только existing node record, сохраняя type, content, extent allocation и child references; tmpfs rename обновляет только in-memory metadata. Case-only spelling changes допустимы, поскольку existing lookup того же object не считается collision.

`MYOS_SYS_VFS_MOVE` (35) намеренно использует тот же fixed two-path request shape, но имеет отдельную семантику. Его dispatcher также validates оба complete NUL-terminated paths до VFS resolution. `vfs_move_object()` принимает только mutable **regular file**, existing writable destination directory и absent final target. Он сохраняет file data и extents, изменяя только metadata existing node: parent и basename; directories не перемещаются, а copy-delete не используется. Persistent source и destination должны разрешаться в один top-level persistent move anchor (`/users/...` — один anchor, а `/system/data/...` и `/system/config/...` — отдельные); tmpfs moves остаются внутри hierarchy `/temp/...`. Cross-anchor, persistent-to-tmpfs, read-only, malformed, missing и overwrite requests fail без VFS mutation. При persistent metadata-store failure in-memory record восстанавливается до return, но current disk format не имеет crash-transaction protocol; этот contract нельзя описывать как crash-atomic.
...
| `main` | Активная экспериментальная QEMU-validated integration line для GUI и user-program work. Compact полностью validated changes могут коммититься прямо; для более рискованной VFS/ABI work, экспериментов или multipart changes используется short-lived branch. |
| `console-stable` | Текущий стабильный console baseline на `v0.12.1-console`. Обновляется только отдельным, намеренно проверенным maintenance decision. |
| `feature/gui` | Историческая GUI integration branch. Сохраняется после локальной интеграции до отдельного решения об архивировании. |

Проверенный commit `main` публикуется только после отдельного approval. Пока не изменится policy physical-PC validation, любой публичный artifact из `main` — отдельно подтверждённый **Pre-release**, но не stable Release; обычные commits не создают tags или releases. Build artifacts `myos.iso` и `myos.img` — release attachments, а не Git-tracked source files.
...
Bounded mouse-first desktop launcher, открываемый bare `startgui` (с `startgui home` как alias, `startgui projects` как bounded Project Hub entry для `/users/myos/projects` и bare `startgui project <name>` как direct revalidated existing entry **Project Workbench** `/users/myos/projects/<name>` (только names из 1–31 ASCII letters/digits/`-`/`_`; его exact suffix `edit` повторно проверяет regular writable `main.mya` перед входом в existing editor; его exact read-only suffix `status` выводит только fixed source/build/package regular-file state через не более 128 probes на parent; его exact suffix `build` повторно проверяет только fixed regular `main.mya`, передаёт established assembler только этот source и fixed output `main.elf`, завершает GUI для console completion и возвращает assembler status через `startgui`; его exact suffix `run [arguments]` повторно проверяет только fixed regular `main.elf`, запускает только этот already allowlisted output с не более existing native tail из 127 visible bytes, затем возвращает child status после завершения GUI; его exact suffix `install` повторно проверяет только этот fixed regular output, запускает established installer только для fixed `/apps/<name>/main.elf`, возвращает installer status после завершения GUI и сохраняет package replacement; его exact suffix `uninstall` повторно проверяет только fixed regular `/apps/<name>/main.elf`, удаляет только этот package output, сохраняя source/build и GUI active, и не создаёт child; его exact suffix `clean` повторно проверяет только fixed regular `<project>/main.elf`, удаляет только этот build output, сохраняя source/package и GUI active, и не создаёт child; его exact suffix `new [hello|args|empty]` принимает только bounded absent name и fixed starter selection, включая editable zero-length `empty`, создаёт только fixed `main.mya`, отклоняет existing directory и откатывает только own partial source/directory state после later ordinary failure; его exact suffix `new [hello|args|empty] edit` использует ту же bounded create-only grammar, затем после успеха выбирает для existing editor только новый fixed source; ordinary editor-handoff failure сохраняет этот completed workspace; его exact suffix `new [hello|args|empty] build` использует ту же bounded create-only grammar, затем после успеха запускает established assembler лишь для новой fixed pair `main.mya → main.elf`, завершает GUI для console completion и сохраняет completed workspace после assembler failure; его exact suffix `new [hello|args|empty] install` использует ту же bounded create-only grammar, затем запускает established assembler лишь для новой fixed source/output pair и только после regular build result запускает established installer лишь для matching package, завершая GUI для child completion и сохраняя completed workspace после build либо installer failure; его exact suffix `remove` немедленно повторно проверяет exact project directory, разрешает только regular `main.mya`, если он присутствует, и absent `main.elf`, удаляет source, затем empty directory, сохраняет installed package, остаётся в GUI и не заявляет crash-transactional deletion; invalid/absent project, source, output либо package target сообщает без viewer fallback)), имеет четыре compact fixed tiles с File Workspace v1; `FILES` начинает в `/users/myos/`, показывает полный current logical path в title окна, проходит logical VFS по four-entry pages, безопасно открывает read-only virtual/system records, предлагает bounded actions `[NEW FILE]`, `[NEW FOLDER]`, `[DELETE]`, `[COPY]`, `[RENAME]` и file-only `[MOVE]` в writable roots, а bounded read-only `[SEARCH]` — в каждом browsable root, открывает new file в completed 16 KiB GUI editor, обновляется после new directory, требует named second-Enter confirmation перед удалением file или empty directory, stream-копирует один regular file до 64 KiB в absent same-directory target без overwrite и переименовывает existing mutable object только внутри current directory без copy content или move children и перемещает один regular file только обновлением metadata в existing writable absolute directory без overwrite, сохраняя basename и оставаясь внутри одного persistent move anchor либо `/temp`. До четырёх verified package tiles `/apps/<name>/main.elf`, каждый с printable name не длиннее 15 characters и одинаковой проверкой в framebuffer и user-space resolver, его per-window controls подъёма по title bar и закрытия, bounded footer indicator `FOCUS HOME/SYSTEM/NOTES/MONITOR` и standard keyboard fallback `Alt+Tab`/`Alt+F4`/`Esc`/`Ctrl+Q` без legacy single-letter GUI commands, SDK VFS subset, direct shell `cp`, `wc`, `grep`, `tree`, `find`, `head`, `sort`, `tail` и `stat`, backed by live developer tools, native build, bounded control-flow, argument forwarding, input/time и general text-editor milestones завершены: `asm` формирует x86_64 `ET_EXEC` с fixed private data segment из `.mya` source; shell `build` предоставляет project workflow; `args`, `input`, `time`, `set <0..255>`, `not`, `neg`, `inc`, `dec`, `parity`, `test <0..255>`, `and <0..255>`, `or <0..255>`, `xor <0..255>`, `shl <1..7>`, `shr <1..7>`, `rol <1..7>`, `ror <1..7>`, `add <0..255>`, `sub <0..255>`, `mul <0..255>`, `div <1..255>`, `mod <1..255>`, `store <0..7>`, `load <0..7>`, `cmp <0..7>`, `swap <0..7>`, `label name:`, `jump name`, `jump_if_zero name`, `jump_if_nonzero name` и `jump_if <0..255> name` компилируются в bounded forward-only code с восемью private byte variables. `not`/`and`/`or`/`xor` требуют initialized byte accumulator и обновляют его восемь bits; operand-free `neg` имеет тот же prerequisite и заменяет его two’s-complement результатом modulo 256; operand-free `inc` имеет тот же prerequisite и увеличивает его modulo 256; operand-free `dec` имеет тот же prerequisite и уменьшает его modulo 256; operand-free `parity` нормализует even byte parity в one или odd parity в zero; `test <0..255>` имеет тот же prerequisite и нормализует его byte-wise intersection в zero или one; `shl`/`shr` требуют его и логически сдвигают byte на 1–7 positions; `rol`/`ror` требуют его и циклически поворачивают byte в том же диапазоне; `add`/`sub`/`mul` также требуют его и wrap modulo 256; `div` имеет то же prerequisite, отклоняет zero и сохраняет unsigned integer quotient; `mod` имеет то же prerequisite, отклоняет zero и сохраняет unsigned remainder; `cmp` сравнивает этот accumulator с private slot и превращает equality в zero, а inequality в one; `swap` требует тот же initialized accumulator и обменивает его byte с одним private slot. Direct `edit <absolute-file>` предоставляет cursor-based multi-line editing для ordinary files и `.mya` source с all-in-memory limit 4 KiB; его contract описан в [TEXT_EDITOR_RU.md](TEXT_EDITOR_RU.md). Будущая native work должна сохранять established storage, ABI и control-flow limits; C frontend или general linker не планируются до стабилизации этих limits. Не переносить GUI, MYPFS004 или native-toolchain work в `console-stable` без explicit maintenance decision; новая работа возвращается в `main` только через задокументированный QEMU validation workflow.

`newproj <name> [hello|args|empty] [edit|build|run|install]` — bounded project entry point. Он принимает один component из 1–31 ASCII letters/digits/`-`/`_` и optional exact starter name: `hello` — default runnable template, `args` — fixed bracketed native-argument template, а `empty` создаёт editable regular zero-length source. После selection принимается только exact final `edit`, `build`, `run` либо `install`. Он проверяет оба до VFS mutation, создаёт `/users/myos/projects/<name>/main.mya`, никогда не перезаписывает existing project и удаляет только собственное partial setup state при последующем ordinary setup failure. После успеха `edit` запускается лишь для нового fixed source, `build` вызывает established fixed project pair `main.mya → main.elf`, `run` сначала повторно проверяет regular new build output перед запуском лишь этого established bounded project output, а `install` сначала повторно проверяет regular new build output перед вызовом established fixed pair `main.elf → /apps/<name>/main.elf`; later editor-launch, build, run либо install failure сохраняет completed project. `editproj <name>` строит fixed source/editor path (`main.mya`), `buildproj <name>` строит source/output pair (`main.mya` → `main.elf`), `runproj <name> [arguments]` строит fixed generated output path, повторно проверяет его regular type и без mutation делегирует existing foreground loader, передавая не более existing native argument tail из 127 visible bytes, а `installproj <name>` — output/package pair (`main.elf` → `/apps/<name>/main.elf`), `uninstallproj <name>` повторно проверяет и удаляет только existing regular fixed installed output (`/apps/<name>/main.elf`), сохраняя source/build и сообщая absent без mutation, `projlist` read-only перечисляет не более 128 entries `/users/myos/projects`, фильтрует valid project directories и выводит те же три fixed source/build/package rows каждого; `projstatus <name>` выводит эти entries через не более 128 VFS-list probes на parent directory, а `cleanproj <name>` удаляет только existing regular fixed build output (`main.elf`) после revalidation; `rmproj <name>` требует absent build, принимает только два known project entries, удаляет regular source, если он существует, а затем его empty fixed project directory, сохраняя installed package; `editproj`, `buildproj` и `installproj` делегируют established programs `edit`, `asm` и `install`, сохраняют их error behavior и сохраняют explicit package-replacement behavior installer. Команды используют существующие VFS requests, cleanproj сохраняет source и installed package и не заявляют crash-transactional persistence. Kernel loader распознаёт тот же exact bounded project output path и всё ещё отклоняет другие `/users/...` paths. BIOS regression проверяет exact default и selected-starter template bytes, duplicate, unknown-starter и trailing-token rejection без creation, empty-starter shell creation-to-editor save/build/run/cleanup, default-starter shell creation-to-build/run/cleanup, creation-to-run/cleanup и creation-to-install/package-run/cleanup, source-ready/build-package-missing status, shortcut editor entry, direct GUI creation-to-build/run/clean/remove, creation-to-install/package-run/uninstall/clean/remove и existing-project build/install/uninstall/clean/remove с native argument forwarding, package readiness, source/build/package preservation, clean-workspace rejection и missing-output/package GUI rejection, shell direct-run/install, ready build/package status, read-only multi-project overview, uninstall/reinstall source-build preservation, clean/rebuild/direct-run behavior, package preservation и execution, затем project-directory removal с preserved package; UEFI проверяет direct GUI build/run/install/uninstall/reinstall/clean плюс persisted direct workspace removal, затем подтверждает removed selected source/build, retained package и final missing build outputs.

### Project Hub action mapping

`startgui projects` и directory entry `/users/myos/projects` из FILES рисуют **Project Hub** через existing surface `MYOS_GUI_CONTENT_FLAG_BROWSER`. Он сканирует не более 128 entries fixed root, фильтрует только valid project directories, удерживает не более четырёх visible entries и одного next-page probe и использует existing browser action IDs для Refresh, Previous, entries, Next, Status и Project root. Selection entry строит только exact fixed project path и повторно проверяет его directory перед activation Project Workbench. Ни одно action Project Hub не создаёт project, не принимает arbitrary path и не меняет VFS request, syscall, loader rule либо GUI ABI.

### Project Workbench action mapping

Bare `startgui project <name>` рисует **Project Workbench** через existing surface `MYOS_GUI_CONTENT_FLAG_BROWSER`. Его visible content rows 1–9 намеренно совпадают с fixed kernel browser-action mapping: `PARENT`/Refresh, `PREVIOUS`/Edit source, entries 0–3/Build–Uninstall, `NEXT`/Clean build, `CREATE`/Remove workspace и `CREATE_DIRECTORY`/Status. State rows начинаются только после этих actions. Workbench строит и повторно проверяет только existing fixed source, build и package paths; Build, Run (с empty arguments) и Install используют pre-existing console handoff. Successful Workbench removal clears Workbench mode и возвращает к browser `/users/myos/projects`, не оставляя stale action surface. Это только user-surface integration: она не добавляет VFS request, syscall, loader allowance или general path grammar.
