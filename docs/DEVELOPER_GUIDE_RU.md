...
| `make regression` | Создаёт disposable raw-image copy; проверяет BIOS default navigation `startgui` через M/H/N/H/Q и clean return, QMP-injected PS/2 activation centered tile `NOTES` с PPM framebuffer transition, а также retained alias `startgui home`, GUI note edit/save, paced editor-authored 305-byte SDK `cp` copy через 256-byte VFS boundary, exact readback и no-overwrite behavior, console-editor source persistence, legacy native branches, empty и forwarded `args` output, exact-match/fallback behavior инструкции `input`, RTC output `HH:MM:SS` и rejection cases, затем UEFI повтор desktop-home navigation, readback copied target, persisted input/time/argument package execution и GUI enter/exit. |
...
| `make regression` | Disposable-image BIOS default navigation `startgui` через M/H/N/H/Q и clean return, QMP-injected PS/2 activation centered tile `NOTES` с PPM framebuffer transition, а также retained alias `startgui home`, GUI note editing, paced editor-authored 305-byte SDK `cp` copy через VFS chunk boundary, exact readback и overwrite rejection, editor source workflow, legacy zero/nonzero branches, empty и forwarded native `args`, native `input` exact-match/fallback paths, valid RTC `HH:MM:SS` output и rejected targets проходят; UEFI повторяет desktop-home navigation, читает persisted text и copied target, запускает persisted packages, включая input/time/argument programs, и корректно выходит из GUI. |
