...
| GUI mutation boundary | 4 KiB GUI editor открывает только selected existing regular files под `/users/myos/`, `/temp/`, `/system/data/` или `/system/config/`; он не делает `/system/core/`, `/system/live/` или `/apps/` writable. |
