...
В каждом established writable root `[NEW FILE]`, `[NEW FOLDER]` и `[DELETE]` принимают одно непустое printable ASCII name без `/` длиной до 63 bytes. File action создаёт один new empty regular file и открывает GUI editor; folder action создаёт один new empty directory. `[DELETE]` сначала показывает complete named target и удаляет этот file или empty directory только после второго подтверждения **Enter**; `Esc` отменяет действие до mutation. Каждое successful action обновляет browser на месте. Existing paths, read-only roots, non-empty directories и malformed names отклоняются без mutation. Rename, copy/move, package installation и raw-device operations остаются shell-only; используйте `rm`, `cp` и `install` для этих workflows.
...
| `startgui` | `startgui` | Запустить experimental framebuffer GUI. Его footer показывает bounded active surface как `FOCUS HOME`, `FOCUS SYSTEM`, `FOCUS NOTES` или `FOCUS MONITOR`. Нажмите `FILES`, чтобы просматривать logical VFS от `/users/myos/`; rows `[NEW FILE]`, `[NEW FOLDER]` и `[DELETE]` принимают имя до 63 printable ASCII bytes без `/` только в `/users/myos`, `/temp`, `/system/data` или `/system/config`. Первый создаёт new empty file и открывает его в GUI editor; второй создаёт directory и обновляет browser; `[DELETE]` показывает named target и требует второго `Enter`, прежде чем удалить file или empty directory. Existing writable text files до 16 KiB также открываются там. |
