...
`calc` принимает два **signed 64-bit целых числа** и один оператор: `+`, `-`, `*` или `/`. Числа могут начинаться со знака `+` или `-`. Рекомендуемый формат — прямой shell command, поэтому `run` писать не требуется.
