...
Программа выводит `[hello MyOS]`, затем выводит текущее время RTC в формате `HH:MM:SS` и возвращает status `37`. При `run native-args` без parameters она выводит `[]`. Source language supports `args`, `input`, `time`, `set <0..255>`, `store <0..7>`, `load <0..7>`, `label name:`, `write "text"`, `jump name`, `jump_if_zero name`, `jump_if_nonzero name`, `jump_if <0..255> name` and final `exit <0..255>`. `store` сохраняет current condition byte в одном из восьми private slots; `load` восстанавливает его как condition и допустим перед conditional jump. Slots private для running program, zero-initialized, не имеют имён и direct addressing и не добавляют arithmetic. Conditional jump требует более ранний `input`, `set` или `load`; every target must be a defined label located later in source, so loops and backward jumps are rejected. Escapes `\n`, `\r`, `\t`, `\\` and `\"` are available inside text. Например, `set 73; store 2; set 0; load 2; jump_if 73 matched` сохраняет value через slot `2`; slot number вне `0..7` отклоняется. The generated program runs in ring 3 and returns its authored exit status; используйте `help asm` и `help edit` для краткой command help, [Текстовый редактор](TEXT_EDITOR_RU.md) для controls и [Native Build](NATIVE_BUILD_RU.md) для всех bounds и syntax rules.
