...
Без аргумента `startgui` открывает **MYOS DESKTOP** — bounded mouse-first launcher; `startgui home` остаётся alias. Click по `SYSTEM` открывает system message, по `NOTES` — notes, по `EDIT NOTE` — editor default personal note. До четырёх installed packages с `/apps/<name>/main.elf` также появляются под fixed tiles как `OPEN APP`; click по одному запускает программу, закрывает GUI и возвращает её normal output в console. Click по `FILES` запускает путь `/users/myos/`; `startgui projects` открывает **Project Hub**, а выбор `/users/myos/projects` из FILES открывает тот же Hub. Он постранично показывает только valid bounded project directories, открывает selected revalidated entry в **Project Workbench**, предоставляет read-only global status и возвращается к raw File Workspace project root. Bare `startgui project <name>` остаётся direct Workbench entry после revalidation его bounded name из 1–31 characters и directory type. Exact suffix `edit` открывает только revalidated regular writable `main.mya` этого project в existing GUI editor. Exact read-only suffix `status` выводит fixed source, build и installed-package rows `READY <size> bytes`, `MISSING` или `NOT REGULAR` без mutation. Exact suffix `build` повторно проверяет только fixed regular source и запускает established assembler для `main.mya → main.elf` этого project, закрывая GUI, пока result остаётся в console, и затем завершаясь с assembler status. Exact suffix `run [arguments]` повторно проверяет только fixed regular `main.elf` этого project, запускает только этот bounded generated output, передаёт не более existing native tail из 127 visible bytes и затем закрывает GUI, возвращая child status. Exact suffix `install` повторно проверяет только тот же fixed regular output и запускает established installer только для `main.elf → /apps/<name>/main.elf` этого project, закрывая GUI и возвращая installer status; replacement existing package остаётся intentional. Exact suffix `uninstall` повторно проверяет только fixed regular `/apps/<name>/main.elf`, удаляет только этот package output, сохраняя project source/build, и оставляет GUI active с narrow result status. Exact suffix `clean` повторно проверяет только fixed regular `<project>/main.elf`, удаляет только этот generated output, сохраняя source/package, и оставляет GUI active с narrow result status. Exact suffix `new [hello|args|empty]` принимает только bounded name и fixed default `hello`, `args` либо editable zero-length starter `empty`, создаёт только `<project>/main.mya`, отклоняет existing project и откатывает только own partial creation state; GUI остаётся active с narrow result status. Exact suffix `new [hello|args|empty] edit` использует тот же creation contract и после успеха открывает в existing editor только новый fixed source; later editor-handoff failure сохраняет completed project. Exact suffix `new [hello|args|empty] build` использует тот же creation contract и после успеха запускает established assembler только для новой fixed pair `main.mya → main.elf`; GUI завершается для console completion, а later assembler failure сохраняет completed project. Exact suffix `new [hello|args|empty] install` использует тот же creation contract, собирает только новую fixed source/output pair и только при regular output устанавливает лишь matching package `/apps/<name>/main.elf`. GUI завершается для child completion; later build либо installer failure сохраняет completed project. Exact suffix `remove` немедленно повторно проверяет exact project directory, принимает только regular `main.mya` при absent `main.elf`, удаляет этот source, затем empty directory, сохраняет `/apps/<name>/main.elf` и оставляет GUI active с narrow result status; он не является crash-transactional. Invalid либо absent project request сообщает `UNABLE TO OPEN PROJECT`; missing или non-regular direct source сообщает `UNABLE TO OPEN PROJECT SOURCE`, missing или non-regular direct output — `UNABLE TO OPEN PROJECT OUTPUT`, а missing или non-regular package — `UNABLE TO OPEN PROJECT PACKAGE`, без открытия viewer. Title его окна показывает полный current logical-VFS path и обновляется после parent или child navigation. Click по top-bar `X` выполняет выход. Launcher и window actions выполняются только мышью. Сохранившиеся GUI-level keyboard shortcuts: `Alt+Tab` переводит focus на следующее видимое окно, `Alt+F4` закрывает focused window, `Esc` возвращает или отменяет, а `Ctrl+Q` выходит. Для personal note можно передать absolute path:
...
# Open Project Workbench for an existing project:
...
# Remove only matching installed package while source/build stay intact:
startgui project native-args uninstall
# Remove only generated build output while source/package stay intact:
startgui project native-args clean
# Remove only clean bounded workspace; installed package stays intact:
...
Используйте `startgui projects` для bounded paged list valid project workspaces. **Refresh**, **Previous**, **Next** и **Status** переиспользуют existing browser action surface; click по project entry повторно проверяет только этот directory и открывает его Project Workbench. **Project root** возвращает к raw File Workspace `/users/myos/projects`. Hub не создаёт projects и не принимает arbitrary filesystem paths, а `startgui projects status` остаётся read-only overview command.
