...
Откроется окно QEMU. При обычной загрузке MyOS покажет сообщение о трёхсекундном countdown и **автоматически** запустит `/init`. Затем откроется пользовательская оболочка:
