...
`make smoke` and `make regression` do not establish networking, USB HID, SMP, production security, long-duration stress reliability, physical-hardware compatibility or a full native C toolchain. They are release-stabilization evidence for the currently implemented GUI including its bounded desktop-home launcher, MYPFS004, public SDK VFS copy workflow, and the restricted native development workflow only.
