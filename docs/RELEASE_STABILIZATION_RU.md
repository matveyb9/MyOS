...
| `make regression` | BIOS GUI note create/edit/save, paced console-editor ordinary text save/readback, SDK `cp` copying 305-byte file через 256-byte VFS boundary с overwrite rejection, editor-authored `.mya` source → `build` → `install` → `run`, conditional rejection cases, затем UEFI text/copied-file/native persistence и clean GUI enter/exit. | Создаёт temporary copy `myos.img` и удаляет её после проверки. Рабочий image пользователя не изменяется. |
