...
| 15 | `[x]` | File Workspace v1 | Compact `FILES` начинает в `/users/myos/`, свободно проходит logical VFS через four-entry pages, безопасно просматривает system/inventory records и вызывает 1 KiB GUI editor только для writable roots. BIOS/UEFI QMP regression покрывает вход по tile, parent navigation и CPIO-backed traversal `/system/core/apps`. |
| 16 | `[x]` | GUI editor 1 KiB expansion | Fixed GUI content ABI теперь поддерживает viewer/editor payload 1 024 bytes. Active GUI-session syscall использует dedicated mapped-range copy, а ordinary syscall I/O остаётся ограничен 512 bytes; ring 3 выполняет не более четырёх VFS reads или writes по 256 bytes. BIOS проверяет GUI save/readback 1 KiB, а UEFI — exact persisted content. |
