...
| `gui/bringup` | Изолированная разработка framebuffer GUI и user-program platform. | `[~]` Preview checkpoint `v0.12.2-gui-preview` и GitHub Pre-release `v0.13.0-gui-rc.1` опубликованы; MYPFS004, persistent ELF execution, внешний SDK и restricted native toolchain развиваются без слияния в `main`. |
...
| 7 | `[ ]` | Следующее расширение native toolchain | Ограниченные conditional jumps на основе явных compare/set operations, затем дополнительные syscalls, multi-line editor и small linker. После стабилизации этих шагов можно оценить более полное C subset, базовую C-библиотеку и build scripts. GCC/Clang не являются первым шагом. |
...
Native build workflow, labels/forward-only jumps, GUI pointer-refresh hardening и automated release-stabilization baseline завершены в `gui/bringup`: MYPFS004 VFS предоставляет единый корень с `/system`, `/apps`, `/users/myos` и `/temp`, а `build` собирает bounded `.mya` source из `/users/myos/projects/` в native ELF64. `label name:` определяет bounded target, а `jump name` пропускает code только в направлении следующей label; BIOS build/install/run, backward-jump rejection и UEFI persistence подтверждены. `install` явно переносит output в `/apps/<name>/main.elf`, после чего `run <name>` создаёт отдельный ring-3 task. `make smoke` confirms raw-image BIOS/UEFI boot markers, `make regression` uses a disposable image to confirm GUI note save, forward-jump native package creation, backward-target rejection и UEFI readback, а `make release-check` produces clean-rebuild source/artifact evidence. Следующий merge-oriented GUI milestone по-прежнему требует physical x86_64 PC smoke test, final release scope и explicit decision о новом immutable GUI tag. Параллельный следующий native-toolchain milestone — ограниченные conditional jumps и затем удобство multi-line project editing; личная установка приложений (`/users/myos/apps`) остаётся отдельным будущим расширением. Preview `v0.12.2-gui-preview` и Pre-release `v0.13.0-gui-rc.1` не сливаются автоматически в `main`; `main` и `console-stable` сохраняют console-only scope. Исходные `myos.iso` и `myos.img` продолжают собираться командой `make all img`.
