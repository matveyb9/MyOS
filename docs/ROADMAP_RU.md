...
| 15 | `[x]` | File Workspace v1 | Compact `FILES` начинает в `/users/myos/`, свободно проходит logical VFS через four-entry pages, безопасно просматривает system/inventory records и вызывает existing 128-byte GUI editor только для writable roots. BIOS/UEFI QMP regression покрывает вход по tile и parent navigation. |
