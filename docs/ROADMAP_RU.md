...
| 11 | `[x]` | Bounded default desktop launcher | Bare `startgui` показывает fixed keyboard launcher `MYOS DESKTOP`; `startgui home` — compatibility alias: `M` открывает system message, `N` открывает notes, `E` открывает selected-note editor, `H` возвращает home, а `Q` возвращает в shell. Он использует existing single GUI session и bounded viewer/editor state. `make regression` проверяет default M/H/N/H/Q navigation и clean return в BIOS и UEFI, а также retained BIOS alias. [GUI_BRINGUP_RU.md](GUI_BRINGUP_RU.md) фиксирует contract. |
