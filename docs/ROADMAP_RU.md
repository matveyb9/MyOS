...
| `[x]` | Keyboard interaction | Compact modifier fallback: `Alt+Tab` для focus, `Ctrl+B` для desktop return и `Alt+F4` для выхода; editor сохраняет `Ctrl+S` и `Esc`. |
...
| 13 | `[x]` | Standard modifier hotkeys | Temporary `M`/`N`/`E`/`H`/`Q`, pointer, numeric и reset letter controls удалены. `Alt+Tab` переключает focus, `Ctrl+B` возвращает home, а `Alt+F4` выходит; `Ctrl+S`/`Esc` остаются controls editor. QMP BIOS/UEFI regression вводит все три GUI-level modifier shortcuts. |
