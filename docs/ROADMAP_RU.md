...
| `[x]` | User utilities | `hello`, `sleeper`, `orphaner`, `safety`, `argshow`, `calc`, `pipewrite`, `piperead`, `wc`, `grep`, `edit`, `tree`, `find`, `head`, `tail`, `stat`. `tree` и `find` — read-only bounded logical-VFS explorer/search utilities с fixed recursion/output limits; `head` читает начало одного absolute file через chunks 256 bytes с запросом 1–64 lines и output cap 4 KiB; `tail` stream-читает один absolute file, сохраняя только его последние 4 KiB для запроса 1–64 trailing lines; `stat` выводит type и size через bounded parent-directory scan. |
...
| 15 | `[x]` | File Workspace v1 | Compact `FILES` начинает в `/users/myos/`, свободно проходит logical VFS через four-entry pages, безопасно просматривает system/inventory records и вызывает 4 KiB GUI editor только для writable roots. BIOS/UEFI QMP regression покрывает вход по tile, parent navigation и CPIO-backed traversal `/system/core/apps`. |
| 16 | `[x]` | GUI editor 4 KiB expansion | Fixed GUI content ABI теперь поддерживает viewer/editor payload 4 096 bytes. Active GUI-session syscall использует dedicated mapped-range copy, а ordinary syscall I/O остаётся ограничен 512 bytes; ring 3 выполняет не более шестнадцати VFS reads или writes по 256 bytes. Deterministic initramfs fixture 4 KiB копируется через existing SDK `cp` path, затем BIOS проверяет GUI save/readback, а UEFI — exact persisted content. |
