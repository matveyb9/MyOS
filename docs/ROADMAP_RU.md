...
GUI release decision принят: immutable preview tag фиксирует проверенный framebuffer scope, но не объявляет GUI production-ready и не меняет stable console baseline. После checkpoint реализованы persistent ELF execution, MyOS SDK, MYPFS004, restricted native `asm`/`build` workflow и pointer-refresh hardening. Следующий merge-oriented приоритет — отдельный GUI release-stabilization pass: clean BIOS/UEFI checklist, physical x86_64 PC smoke test, final release scope и explicit decision о новом tag. GUI не требуется сливать в `main`, чтобы продолжать development environment, но stable merge не должен предшествовать этой проверке.
...
Native build workflow и GUI pointer-refresh hardening завершены в `gui/bringup`: MYPFS004 VFS предоставляет единый корень с `/system`, `/apps`, `/users/myos` и `/temp`, а `build` собирает bounded `.mya` source из `/users/myos/projects/` в native ELF64. `install` явно переносит output в `/apps/<name>/main.elf`, после чего `run <name>` создаёт отдельный ring-3 task; source-to-package execution подтверждён в BIOS, после fresh remount и в UEFI. Следующий merge-oriented milestone — GUI release stabilization: clean BIOS/UEFI checklist, physical x86_64 PC smoke test, final release scope и explicit decision о новом immutable GUI tag. После этого restricted native toolchain может расширяться labels/control flow и удобством multi-line project editing; личная установка приложений (`/users/myos/apps`) остаётся отдельным будущим расширением. Preview `v0.12.2-gui-preview` не сливается в `main`; `main` и `console-stable` сохраняют console-only scope. Исходные `myos.iso` и `myos.img` продолжают собираться командой `make all img`.
