...
| 11 | `[x]` | Bounded desktop-home launcher | `startgui home` показывает fixed keyboard launcher `MYOS DESKTOP`: `M` открывает system message, `N` открывает notes, `E` открывает selected-note editor, `H` возвращает home, а `Q` возвращает в shell. Он использует existing single GUI session и bounded viewer/editor state. `make regression` проверяет M/H/N/H/Q navigation и clean return в BIOS и UEFI. [GUI_BRINGUP_RU.md](GUI_BRINGUP_RU.md) фиксирует contract. |
