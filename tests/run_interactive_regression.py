...
class Guest:
    ...
    def gui_project_hub_and_exit(self):
        start = len(self.output)
        self.send("startgui projects\n")
        self.expect("Started process ", start)
        time.sleep(0.25)
        hub = self.qmp_screendump("project-hub")
        self.require_nonuniform_region(hub, 334, 210, 80, 7, "Project Hub header")
        self.qmp_hotkey("ctrl", "q")
        self.expect("exited with status 0", start)
        self.expect(PROMPT, start)

    def gui_project_hub_select_workbench_and_exit(self):
        start = len(self.output)
        self.send("startgui projects\n")
        self.expect("Started process ", start)
        time.sleep(0.25)
        hub = self.qmp_screendump("project-hub-selection")
        self.require_nonuniform_region(hub, 334, 210, 80, 7, "Project Hub header before selection")
        # Hub project entries occupy fixed browser rows 3..6; select the first revalidated project.
        self.qmp_move(delta_x=-140, delta_y=400 - (248 + 3 * 12 + 6))
        time.sleep(0.10)
        ready = self.qmp_screendump("project-hub-selection-ready")
        self.qmp_left_click()
        time.sleep(0.25)
        workbench = self.qmp_screendump("project-hub-workbench")
        self.require_region_transition(ready, workbench, 330, 208, 280, 48,
                                       "Project Hub selection transition to Project Workbench")
        self.require_nonuniform_region(workbench, 334, 210, 96, 7, "Project Workbench title from Hub")
        self.qmp_hotkey("ctrl", "q")
        self.expect("exited with status 0", start)
        self.expect(PROMPT, start)
    ...
    def gui_direct_project_clean_and_exit(self, project_name):
        ...
        time.sleep(0.25)
        if b"exited with status" in bytes(self.output[start:]):
            raise RegressionFailure(f"GUI clean request unexpectedly ended the session\n{self._tail()}")
        status = self.qmp_screendump("direct-project-clean-status")
        self.require_nonuniform_region(status, 330, 210, 200, 24, "direct project clean status")
        self.qmp_hotkey("ctrl", "q")
        self.expect("exited with status 0", start)
        self.expect(PROMPT, start)

    def gui_direct_project_clean_output_missing_and_exit(self, project_name):
        start = len(self.output)
        self.send(f"startgui project {project_name} clean\n")
        self.expect("Started process ", start)
        time.sleep(0.25)
        if b"exited with status" in bytes(self.output[start:]):
            raise RegressionFailure(f"GUI clean missing-output request unexpectedly ended the session\n{self._tail()}")
        status = self.qmp_screendump("direct-project-clean-output-missing-status")
        self.require_nonuniform_region(status, 330, 210, 200, 24, "direct project clean missing-output status")
        self.qmp_hotkey("ctrl", "q")
        self.expect("exited with status 0", start)
        self.expect(PROMPT, start)

    def gui_direct_project_uninstall_and_exit(self, project_name):
        start = len(self.output)
        self.send(f"startgui project {project_name} uninstall\n")
        self.expect("Started process ", start)
        time.sleep(0.25)
        if b"exited with status" in bytes(self.output[start:]):
            raise RegressionFailure(f"GUI uninstall request unexpectedly ended the session\n{self._tail()}")
        status = self.qmp_screendump("direct-project-uninstall-status")
        self.require_nonuniform_region(status, 330, 210, 200, 24, "direct project uninstall status")
        self.qmp_hotkey("ctrl", "q")
        self.expect("exited with status 0", start)
        self.expect(PROMPT, start)

    def gui_direct_project_uninstall_package_missing_and_exit(self, project_name):
        start = len(self.output)
        self.send(f"startgui project {project_name} uninstall\n")
        self.expect("Started process ", start)
        time.sleep(0.25)
        if b"exited with status" in bytes(self.output[start:]):
            raise RegressionFailure(f"GUI uninstall missing-package request unexpectedly ended the session\n{self._tail()}")
        status = self.qmp_screendump("direct-project-uninstall-package-missing-status")
        self.require_nonuniform_region(status, 330, 210, 200, 24, "direct project uninstall missing-package status")
        self.qmp_hotkey("ctrl", "q")
        self.expect("exited with status 0", start)
        self.expect(PROMPT, start)
...
def run_bios(image_path, work_dir):
    ...
    try:
        ...
        guest.command(f"stat {GUI_MOVE_TARGET_PATH}", "16384 bytes")
        guest.gui_project_hub_and_exit()
        ...
        guest.gui_direct_project_new_and_exit(ARGS_PROJ_NAME, "args")
        guest.command("newproj empty-shell empty", "Created project /users/myos/projects/empty-shell")
        guest.command("stat /users/myos/projects/empty-shell/main.mya", "0 bytes")
        ...
        guest.gui_direct_project_clean_output_missing_and_exit(NEWPROJ_NAME)
        ...
        guest.gui_project_hub_select_workbench_and_exit()
        ...
        guest.gui_direct_project_uninstall_and_exit(NEWPROJ_NAME)
        guest.command(f"projstatus {NEWPROJ_NAME}", "source: READY")
        guest.command(f"projstatus {NEWPROJ_NAME}", "build: READY")
        guest.command(f"projstatus {NEWPROJ_NAME}", "package: MISSING")
        guest.gui_direct_project_install_and_exit(NEWPROJ_NAME)
        guest.command(f"projstatus {NEWPROJ_NAME}", "package: READY")
        guest.gui_direct_project_clean_and_exit(NEWPROJ_NAME)
        guest.command(f"projstatus {NEWPROJ_NAME}", "source: READY")
        guest.command(f"projstatus {NEWPROJ_NAME}", "build: MISSING")
        guest.command(f"projstatus {NEWPROJ_NAME}", "package: READY")
