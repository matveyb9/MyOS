...
Текущая стабильная линия — [`console-stable`](docs/RELEASES_RU.md) на теге [`v0.12.1-console`](docs/RELEASES_RU.md). Ветка `main` — QEMU-validated integration line для графического интерфейса и user-program development; она остаётся **экспериментальной**, поскольку physical-PC validation пока недоступна. Новая функциональная работа начинается в короткоживущей feature-ветке от `main` и возвращается через проверенный QEMU validation gate. Любой публичный artifact из `main` остаётся только **Pre-release**, пока не будет принята policy physical-PC validation. Ветка содержит BIOS/UEFI boot paths, persistent MYPFS004 storage, framebuffer GUI с bounded mouse-first desktop launcher, открываемым через `startgui` и обнаруживающим до четырёх installed tiles `/apps/<name>/main.elf`, compositor-owned clock widget `HH:MM:SS`, обновляемый раз в PIT second через clock-only partial repaint, и live bounded status footer `FOCUS`/`TASKS`/`RUN`, обновляемым вместе с GUI content, per-window controls подъёма по title bar и закрытия, общий bounded text editor, persistent ELF packages, MyOS SDK с bounded public VFS subset, direct shell `cp`, backed by live developer tool, direct bounded `wc` word counting, direct bounded `grep` text search, direct bounded `tree` hierarchy view, direct bounded `find` name search, direct bounded `head` text preview, direct bounded `sort` text ordering, direct bounded `tail` text preview, direct bounded `stat` metadata lookup и packaged safe-write example `sdk-write`, native read-only bounded `tree`, `find`, `head`, `tail`, `sort`, `stat` и 12 KiB `stackprobe` VFS/platform diagnostics, four-page guarded ring-3 user stack размером 16 KiB, встроенный assembler с bounded program-argument forwarding, single-byte input, RTC output `HH:MM:SS`, восемью private byte variables `store`/`load`, bounded bitwise `not`, modular byte `neg`, `inc` и `dec`, bounded leading-zero `clz`, normalized even-parity `parity`, `and`, `or` и `xor`, bounded logical byte shifts `shl`/`shr` и circular byte rotates `rol`/`ror`, modular byte arithmetic `add`/`sub`/`mul`, safe unsigned `div`, bounded unsigned-remainder `mod` и bounded private-slot `cmp` и `swap`, normalized byte-mask `test`, метками, явными condition values, exact-byte comparison и forward-only безусловными или условными переходами, а также read-only System Inventory `/system/live/` с boot, compiled-in driver, device и process records, выводимыми командой `sysinfo`, и File Workspace v1: compact desktop tile `FILES`, который начинает в `/users/myos/`, просматривает полную logical VFS с полным current path в title окна, fixed-column type и byte-size metadata, создаёт named file либо directory из bounded prompts без `/` только в existing writable roots, отображает и фиксирует named file либо empty-directory target до подтверждения удаления вторым Enter, копирует один regular file до 64 KiB в absent same-directory target через bounded VFS chunks, переименовывает existing mutable file или directory внутри того же directory без копирования content или перемещения children, перемещает один regular file только обновлением metadata в existing writable destination directory с сохранением basename и отклонением overwrite за cross-anchor boundaries, открывает new file в GUI editor и обновляет browser после mutation, а также открывает bounded writable text files до 16 KiB в GUI editor через до шестидесяти четырёх неизменных VFS transfers по 256 bytes без ослабления read-only или boot boundaries.

| Линия | Назначение | Состояние |
|---|---|---|
| `console-stable` | Проверенный baseline консольной ОС. | Текущая стабильная линия, тег `v0.12.1-console`. |
| `main` | QEMU-validated integration line для GUI и user-program development. | Экспериментальная; источник новых feature-веток и только Pre-releases. |
...
Склонируйте репозиторий и выберите нужную ветку. Команды ниже выбирают актуальную QEMU-validated integration line.
