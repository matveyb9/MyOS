...
`make regression` использует disposable copy `myos.img`: он покрывает навигацию `startgui home` и clean return в BIOS/UEFI, GUI/editor workflow, 305-byte SDK `cp` copy через VFS request boundary, его no-overwrite rule и UEFI persistence, native forward-only control flow, empty и forwarded native program arguments, input true/fallback branches и valid RTC output `HH:MM:SS`. `make release-check` выполняет только локальную проверку. Он не создаёт tag, GitHub Release или Pre-release.
