...
`make regression` использует disposable copy `myos.img`: он покрывает GUI/editor workflow, native forward-only control flow, input true/fallback branches, valid RTC output `HH:MM:SS` и BIOS-to-UEFI package persistence. `make release-check` выполняет только локальную проверку. Он не создаёт tag, GitHub Release или Pre-release.
